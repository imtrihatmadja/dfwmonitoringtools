...
// ===================== HELPER: AMBIL NILAI AKTUAL INDIKATOR =====================
// Fungsi ini TIDAK bergantung pada ind.indicator_updates (bisa kosong)
// Prioritas: (1) indicator_updates terbaru, (2) ind.actual, (3) 0
function _rptGetActual(ind) {
  const upds = ind.indicator_updates || [];
  if (upds.length > 0) {
    // sort by created_at agar benar-benar paling baru
...
// ===================== HELPER: HITUNG PROGRESS DARI STATE =====================
// Menghitung langsung dari indicators[] dan allActivities[] yang ada di memory,
// tidak bergantung pada currentProject.project_indicators
function _rptCalcProgress() {
...
// Hitung rata-rata % capaian indikator saja (untuk summary cover)
function _rptAvgIndikator() {
  const inds = indicators || [];
...
// Hitung jumlah indikator yang sudah selesai (>= 100%)
function _rptCountDoneIndicators() {
...
// Hitung jumlah aktivitas selesai
function _rptCountDoneActivities() {
  return (allActivities || []).filter(a => a.status === 'Selesai').length;
}
...
async function generateAndPrint(lang) {
  ...
  const T = lang === 'id' ? {
    ...
    allDone: 'Semua aktivitas telah selesai atau sedang berjalan',
    ...
    avgInd: 'Rata-rata Capaian Indikator',
  } : {
    org: 'DFW Indonesia',
    ...
    allDone: 'All active activities are completed or in progress',
    ...
    avgInd: 'Avg. Indicator Achievement',
  };
  ...
  // ── KALKULASI UTAMA (perbaikan: tidak pakai currentProject.project_indicators) ──
  const overallPct      = _rptCalcProgress();
  const avgIndPct       = _rptAvgIndikator();
  const doneIndCount    = _rptCountDoneIndicators();
  const doneActCount    = _rptCountDoneActivities();
  ...
  function indStatusBadge(pct) {
    if (pct >= 100) return `<span class="rpt-badge on-track">${lang==='id'?'Tercapai':'Achieved'}</span>`;
  ...
  // Hanya aktivitas yang masih berjalan & belum mulai
  const activeActs = acts.filter(a =>
    a.status === 'Sedang Berjalan' || a.status === 'Belum Mulai'
  );

  // Kumpulkan lessons learned dari catatan aktivitas + update indikator
  ...
  const combinedNotes = [...actNoteTexts, ...indNoteTexts].filter(Boolean);
...
<div class="rpt-page">
  ...
  <div class="rpt-section">
    ...
    <div class="rpt-progress-wrap">
      <div style="flex-shrink:0;text-align:center;min-width:90px;padding-right:8px;border-right:1px solid #e2e8f0">
        <div class="rpt-circle-pct">${overallPct}%</div>
        <div class="rpt-circle-label">${progressLabel(overallPct)}</div>
      </div>
      <div style="flex:1;padding-left:16px">
        <div style="margin-bottom:10px">${progressBar(overallPct)}</div>
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:8px">
          <div style="font-size:9px;color:#64748b;text-align:center;background:#f8fafc;border-radius:6px;padding:7px 4px">
            <strong style="display:block;color:#2563eb;font-size:15px">${avgIndPct}%</strong>
            ${T.avgInd}
          </div>
          <div style="font-size:9px;color:#64748b;text-align:center;background:#f8fafc;border-radius:6px;padding:7px 4px">
            <strong style="display:block;color:#22c55e;font-size:15px">${doneIndCount}/${inds.length}</strong>
            ${T.indDone}
          </div>
          <div style="font-size:9px;color:#64748b;text-align:center;background:#f8fafc;border-radius:6px;padding:7px 4px">
            <strong style="display:block;color:#22c55e;font-size:15px">${doneActCount}/${acts.length}</strong>
            ${T.actDone}
          </div>
          <div style="font-size:9px;color:#64748b;text-align:center;background:#f8fafc;border-radius:6px;padding:7px 4px">
            <strong style="display:block;color:#3b82f6;font-size:15px">${budgetPct}%</strong>
  ...
  const indRows = inds.map((ind, i) => {
    // Gunakan _rptGetActual agar fallback ke ind.actual jika indicator_updates kosong
    ...
    return `<tr>
      ...
      <td>
        <strong>${ind.indicator_name}</strong><br>
        <span style="color:#94a3b8;font-size:9px">${ind.type || 'Output'}</span>
      </td>
      <td style="text-align:center;font-weight:600">${tgtVal.toLocaleString('id-ID')} ${ind.unit || ''}</td>
      <td style="text-align:center;font-weight:700;color:#0f172a">${actVal.toLocaleString('id-ID')} ${ind.unit || ''}</td>
  ...
  const actRows = activeActs.map((act, i) => {
    const actNoteList = notes.filter(n => n.activity_id === act.id);
    const lastNote    = actNoteList.length
      ? actNoteList[actNoteList.length - 1].note
      : T.noData;
...
<div class="rpt-page">
  <div class="rpt-section">
    ...
    ${inds.length === 0
      ? `<div style="text-align:center;padding:20px;color:#94a3b8;font-size:11px">${T.noData}</div>`
      : `<table class="rpt-table">
          <thead><tr>
            ...
            <th style="width:90px">${T.statusCol}</th>
          </tr></thead>
          <tbody>${indRows}</tbody>
          <tfoot>
            <tr style="background:#f1f5f9">
              <td colspan="4" style="text-align:right;font-weight:700;font-size:10px;padding:8px 9px">
                ${lang==='id'?'Rata-rata Capaian':'Average Achievement'}:
              </td>
  ...
  <div class="rpt-section">
    ...
    ${activeActs.length === 0
      ? `<div style="text-align:center;padding:20px;color:#94a3b8;font-size:11px;background:#f8fafc;border-radius:8px">${T.allDone}</div>`
      : `<table class="rpt-table">
          <thead><tr>
            <th style="width:28px">#</th>
            <th>${T.actTitle}</th>
            <th style="width:80px">${T.pic}</th>
            <th style="width:70px">${T.startPlan}</th>
            <th style="width:70px">${T.duePlan}</th>
...
<div class="rpt-page">
    ...
    <table class="rpt-table">
      ...
      <tbody>
        <tr>
          ...
          <td style="text-align:right;font-weight:600;color:#2563eb">${formatRupiah(budgetActual)}</td>
          <td style="text-align:right;font-weight:600;color:${budgetRemaining >= 0 ? '#15803d' : '#b91c1c'}">${formatRupiah(budgetRemaining)}</td>
          <td>${progressBar(budgetPct, budgetPct >= 80 ? '#22c55e' : '#3b82f6')}</td>
        </tr>
      </tbody>
    </table>
    <div style="margin-top:8px;padding:8px 12px;background:#f8fafc;border-radius:6px;font-size:10px;color:#64748b;display:flex;gap:20px">
      <span>${T.remainingBudget}: <strong style="color:#0f172a">${formatRupiah(budgetRemaining)}</strong></span>
      <span>${T.absLabel}: <strong style="color:#2563eb">${budgetPct}%</strong></span>
    </div>
  </div>
  ...
  // ─────────────────────────────────────────────────────
  // Render ke container + toolbar preview
  // ─────────────────────────────────────────────────────
  const fullHTML = `
