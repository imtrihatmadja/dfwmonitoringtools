...
// ============================================================
// IMPACT DATA untuk laporan cetak — semua proyek, grouped per unit
// ============================================================
async function _rptFetchImpactAll() {
  ...
  data.forEach(ind => {
    ...
    if (!grouped[unitKey]) {
      grouped[unitKey] = { unitDisplay: rawUnit, total: 0, count: 0, indicators: [] };
    }
...
<div class="rpt-page">
  ...
  // ─────────────────────────────────────────────────────
  // PAGE 4: Ringkasan Total Dampak (lintas semua proyek)
  // ─────────────────────────────────────────────────────
  function _rptImpactIcon(unit) {
    ...
    if (['kapal','perahu','unit','alat'].some(k => u.includes(k))) return '🚢';
  ...
  const impactEntries = Object.entries(impactAll)
    .sort((a, b) => b[1].total - a[1].total);

  // Grid summary: 4 card per baris
  function buildImpactSummaryGrid() {
    if (!impactEntries.length) return '';
    const cells = impactEntries.map(([unitKey, d]) => {
      const icon  = _rptImpactIcon(unitKey);
      const total = Number(d.total).toLocaleString('id-ID');
      return `<div style="background:#f8fafc;border:1.5px solid #e2e8f0;
        border-radius:10px;padding:14px 10px;text-align:center">
        <div style="font-size:22px;margin-bottom:6px">${icon}</div>
        <div style="font-size:20px;font-weight:800;color:#0f172a;line-height:1.1">${total}</div>
    ...
    return `<div style="display:grid;grid-template-columns:repeat(4,1fr);
      gap:10px;margin-bottom:18px">${cells.join('')}</div>`;
  }

  // Detail tabel per satuan
    ...
    return impactEntries.map(([unitKey, d]) => {
      ...
      const indRows = d.indicators
        ...
        .map((ind, i) => `
          <tr>
            ...
            <td style="font-size:9px;color:#64748b">${ind.project_name}</td>
            <td style="font-size:10px;font-weight:600">${ind.indicator_name}</td>
            <td style="text-align:right;font-weight:700;color:#0f172a;font-size:11px">
      ...
      <div style="margin-bottom:14px;page-break-inside:avoid">
        <div style="font-size:10px;font-weight:800;color:#0f172a;
          border-left:4px solid #2563eb;padding-left:8px;margin-bottom:7px;
          ...
          <span style="font-weight:400;color:#64748b;font-size:9px;margin-left:4px">
            — Total: <strong style="color:#2563eb">
        ...
        <table class="rpt-table">
          <thead><tr>
            <th style="width:24px">#</th>
            <th style="width:110px">${lang==='id'?'Proyek':'Project'}</th>
            <th>${lang==='id'?'Indikator':'Indicator'}</th>
            <th style="text-align:right;width:100px">${lang==='id'?'Realisasi':'Actual'}</th>
          </tr></thead>
          <tbody>${indRows}</tbody>
          <tfoot>
...
<div class="rpt-page">
  <div class="rpt-cover-header" style="padding:22px 24px 18px">
    ...
    <div class="rpt-proj-name" style="font-size:17px">
      🌍 ${lang==='id'?'Ringkasan Total Dampak':'Total Impact Summary'}
    </div>
    <div class="rpt-proj-code">
      ${lang==='id'
        ? 'Akumulasi capaian indikator berdasarkan satuan — lintas semua proyek aktif'
        : 'Cumulative indicator achievements grouped by unit — across all active projects'}
    </div>
  </div>
  ...
  <div class="rpt-section">
    ...
    ${buildImpactSummaryGrid()}
  </div>

  <div class="rpt-section">
    <div class="rpt-section-title">
      📋 ${lang==='id'?'Rincian Kontribusi per Indikator':'Contribution Detail per Indicator'}
    </div>
    ${buildImpactDetailTables()}
  </div>

  <div class="rpt-page-footer">
    <span>${T.org} — ${lang==='id'?'Ringkasan Total Dampak':'Total Impact Summary'}</span>
