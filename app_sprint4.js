...
// Fungsi generik: load staff dari staff_roster ke <select> tertentu
...
// Wrapper untuk dropdown Aktivitas (act-pic)
async function loadStaffDropdown() {
    await _loadStaffToSelect("act-pic");
}

// Wrapper untuk dropdown Sub-Aktivitas (sub-task-pic)
...
window.openActModal = function (id) {
    ...
    async function loadSubActivities(activityId) {
        ...
        if (!activityId) {
            listContainer.innerHTML = "<p>Tambah aktivitas dulu untuk menambahkan sub-aktivitas.</p>";
            return;
        }
        ...
        if (error || !data || data.length === 0) {
            listContainer.innerHTML = "<p>Belum ada sub-aktivitas.</p>";
        ...
        listContainer.innerHTML = data.map((task, idx) => {
            ...
            return `\n                <div class=\"sub-task-item\">\n                    <p>${idx+1}. ${esc(task.title)}</p>\n                    <span style=\"background:${priorityColor}\">${task.priority}</span>\n                    <span style=\"background:${statusColor}\">${task.status}</span>\n                    <span>PIC: ${esc(task.pic)}</span>\n                    ${task.due_date ? ` | ${fmtDate(task.due_date)} ` : ''}\n                    <button onclick=\"editSubActivity(${task.id})\">Edit</button>\n                    <button onclick=\"deleteSubActivity(${task.id})\">Hapus</button>\n                </div>\n            `;
        }).join("");
        addBtn.disabled = false;
    }
    ...
    window.editSubActivity = async function(taskId) {
        ...
        if (error || !data) {
            alert("Gagal memuat sub-aktivitas");
            return;
        }
    ...
    document.getElementById("saveSubActivityBtn").addEventListener("click", async () => {
        ...
        if (!activityId) {
            alert("Error: Activity ID tidak tersedia");
            return;
        }
        if (!title) {
    ...
    // Setelah modal terbuka, load dropdown staff untuk Aktivitas
    loadStaffDropdown();
};
