      ...
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-bottom:12px">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
          <select id="learnFilterKegiatan" onchange="applyLearnFilter(this.value)" style="font-size:12px;padding:5px 10px;border:1px solid #d1d5db;border-radius:8px;color:#374151">
            <option value="">Semua Kegiatan</option>${(LEARN.activities||[]).map(a=>`<option value="${_esc(a.id)}" ${LEARN.filterKegiatan===a.id?'selected':''}>${_esc(a.title || a.title || a.name || '' || '')}</option>`).join('')}
          </select>
...
window.loadLearningLoop = async function(caseId){
  ...
  try {
    ...
    const [rMem, rRef, rLes, rAct] = await Promise.all([
      ...
      _lc().from('project_activities').select('id,title,project_name').eq('project_name', (window.currentProject && window.currentProject.name) || LEARN.caseName).order('sort_order', { ascending:true }).order('sort_order', { ascending:true }).order('created_at', { ascending:true }),
    ]);
