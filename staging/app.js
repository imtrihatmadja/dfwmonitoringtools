...
window.populateDetailLearningActivities = async function(){
  ...
  if (!cp) return;
  const { data, error } = await (window.client || client).from("activities").select("id,name").eq("project_id", cp.id).order("due_date", { ascending: true });
  if (error) return;
  sel.innerHTML = '<option value="">— Pilih kegiatan (opsional) —</option>' + (data||[]).map(a => `<option value="${a.id}">${a.name}</option>`).join('');
};
