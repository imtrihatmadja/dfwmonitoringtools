;(() => {
  if (window.__KB_ALREADY_LOADED__) {
    console.warn('knowledge.js sudah dimuat, skip duplicate load.');
    return;
  }
  window.__KB_ALREADY_LOADED__ = true;
  ...
  function buildRssPanel() {
    const savedOptions = kbFeeds.map(feed =>
      `<option value="${esc(feed.feed_url)}">${esc(feed.feed_name)}</option>`
    ).join('');

    const defaultOptions = DEFAULT_FEEDS.map(feed =>
      `<option value="${esc(feed.url)}">${esc(feed.name)}</option>`
    ).join('');
    ...
    return `
      <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:18px;box-shadow:0 1px 4px rgba(0,0,0,.05);">
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:14px;">
          ...
          <button type="button" class="btn-primary btn-sm" onclick="kbFetchRss()">Muat</button>
          <button type="button" class="btn-secondary btn-sm" onclick="kbSaveFeed()">Simpan</button>
        </div>
          ...
          <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:16px;display:flex;gap:14px;box-shadow:0 1px 4px rgba(0,0,0,.04);">
            ...
            <div style="flex:1;min-width:0;text-align:left;">
              ...
              <div style="display:flex;gap:8px;flex-wrap:wrap;">
                ...
                <button type="button" class="btn-primary btn-sm" onclick="kbSaveArticleByIndex(${index})">Simpan</button>
              </div>
            </div>
          </div>
        `).join('')}
        ...
        ${kbArticles.map(a => `
          <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:16px;box-shadow:0 1px 4px rgba(0,0,0,.04);">
            ...
            <div style="display:flex;gap:8px;flex-wrap:wrap;">
              ...
              <button type="button" class="btn-secondary btn-sm" style="color:#dc2626;" onclick="kbDeleteArticle('${a.id}')">Hapus</button>
            </div>
          </div>
        `).join('')}
      </div>
    ...
    return `
      <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:20px;box-shadow:0 1px 4px rgba(0,0,0,.05);">
        ...
        <div style="display:flex;gap:8px;margin-bottom:18px;">
          ...
          <button type="button" class="btn-primary btn-sm" onclick="kbAddTopic()">Tambah</button>
        </div>
        ...
        <div style="display:flex;flex-direction:column;gap:8px;">
          ${kbTopics.length ? kbTopics.map(t => `
            <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;">
              ...
              <button type="button" class="btn-remove" onclick="kbDeleteTopic('${t.id}')" title="Hapus">×</button>
            </div>
          `).join('') : `
          ...
          ${kbFeeds.length ? kbFeeds.map(f => `
            <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;gap:10px;">
              ...
              <div style="display:flex;gap:6px;flex-shrink:0;">
                <button type="button" class="btn-edit" onclick="kbLoadFeed('${esc(f.feed_url)}')" title="Muat feed ini">↗</button>
                <button type="button" class="btn-remove" onclick="kbDeleteFeed('${f.id}')" title="Hapus">×</button>
              </div>
            </div>
          `).join('') : `
  ...
  function tryAutoInitKB() {
    const tab = document.getElementById('tab-knowledge');
    if (tab && tab.classList.contains('active') && !tab.dataset.kbInit) {
