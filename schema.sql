...
-- Policy: User hanya bisa baca/edit profil sendiri
CREATE POLICY "users_own_profile" ON user_profiles
  FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- Policy: Admin bisa baca/edit semua profil
DROP POLICY IF EXISTS "admin_all_profiles" ON user_profiles;
CREATE POLICY "admin_all_profiles" ON user_profiles
  FOR ALL USING (
    EXISTS (SELECT 1 FROM user_profiles up2 WHERE up2.user_id = auth.uid() AND up2.role = 'admin')
  );

-- Policy: Semua user authenticated bisa baca semua profil (untuk fitur user management)
DROP POLICY IF EXISTS "read_all_profiles" ON user_profiles;
CREATE POLICY "read_all_profiles" ON user_profiles
  FOR SELECT USING (auth.uid() IS NOT NULL);

-- Policy: User authenticated bisa INSERT profil sendiri (penting untuk auto-create saat login pertama)
