...
-- Tambahkan kolom email ke user_profiles (aman jika sudah ada)
ALTER TABLE user_profiles ADD COLUMN IF NOT EXISTS email text;

-- Update email dari auth.users untuk semua profil yang belum punya email
...
-- Digunakan oleh fitur "Tambah Pengguna" di frontend
-- (mengambil auth.users.id berdasarkan email)
...
-- Grant akses ke fungsi ini untuk authenticated users
GRANT EXECUTE ON FUNCTION get_user_id_by_email(text) TO authenticated;

-- ===================== SETUP SUPER ADMIN: admin@dfw.or.id =====================
-- Jalankan ini SETELAH mendaftarkan email admin@dfw.or.id di Supabase Auth:
--
--   Supabase Dashboard → Authentication → Users → Add User
--   Email    : admin@dfw.or.id
--   Password : (buat password kuat)
--
-- Kemudian jalankan query berikut untuk menjadikannya Super Admin:
--
-- INSERT INTO user_profiles (user_id, email, name, role, avatar_initial)
-- SELECT id, email, 'Admin DFW', 'admin', 'A'
-- FROM auth.users
-- WHERE lower(email) = 'admin@dfw.or.id'
-- ON CONFLICT (user_id) DO UPDATE
--   SET role = 'admin',
--       email = EXCLUDED.email,
--       name = COALESCE(user_profiles.name, 'Admin DFW'),
--       avatar_initial = 'A';
--
-- ===================== RLS POLICY untuk user_profiles =====================
-- Policy agar admin dapat membaca & mengelola semua profil
