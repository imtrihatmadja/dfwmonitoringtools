...
function getLatestActual(ind) {
  // Selalu gunakan nilai dari entry TERAKHIR indicator_updates jika ada
...
function calcAvgAktivitas(proj) {
  const acts = proj.activities_summary || [];
...
function renderIndicatorUpdatePanel(proj) {
  ...
  container.innerHTML = inds.map((ind, i) => {
    // Gunakan nilai aktual dari entry TERAKHIR indicator_updates jika ada,
    // fallback ke kolom actual di project_indicators
    ...
    // Warna progress sesuai pencapaian
    ...
    return `
      <div class="ind-update-card">
        ...
        <div class="ind-capaian-display">
          ...
          <div class="ind-capaian-pct-badge" style="background:${pctColor}15;color:${pctColor};border:1px solid ${pctColor}40">
            ${pct}%
          </div>
        </div>
        ...
        <!-- Input update -->
        <div class="ind-update-inputs" style="margin-top:10px">
          <div class="form-group">
            <label>Nilai Capaian Baru</label>
            <input type="number" id="upd-actual-${i}" value="${currentActual}" placeholder="0"
              oninput="updateIndPct(${i}, ${target})"
              data-ind-id="${ind.id}"
              data-ind-name="${escHtml(ind.indicator_name)}"
              data-unit="${escHtml(ind.unit || "")}">
          </div>
          <div class="form-group">
            <label>% Capaian (auto)</label>
            <input type="number" id="upd-pct-${i}" value="${pct}" readonly
              style="background:#f8fafc;font-weight:700;color:${pctColor}">
          </div>
        </div>
        <div class="form-group" style="margin-top:8px">
          <label>Catatan Update <span style="font-weight:400;color:#94a3b8">(opsional)</span></label>
...
document.getElementById("saveIndicatorUpdatesBtn").addEventListener("click", async () => {
  ...
  try {
    for (let i = 0; i < inds.length; i++) {
      ...
      // Ambil nilai aktual terakhir dari histori (bukan dari kolom actual saja)
      const lastUpdate = ind.indicator_updates && ind.indicator_updates.length
        ? ind.indicator_updates[ind.indicator_updates.length - 1] : null;
      const prevActual = lastUpdate ? Number(lastUpdate.actual_value) : Number(ind.actual) || 0;

      // Update kolom actual di project_indicators
      await client.from("project_indicators").update({ actual: newActual }).eq("id", ind.id);

      // Hanya insert ke indicator_updates jika nilai BERUBAH atau ada catatan baru
      if (newActual !== prevActual || note) {
        await client.from("indicator_updates").insert({
          indicator_id   : ind.id,
      ...
      // Kosongkan textarea catatan setelah simpan
      if (noteEl) noteEl.value = "";
    }
    msg.textContent = saved > 0
      ? `✅ ${saved} indikator diperbarui!`
      : "ℹ️ Tidak ada perubahan nilai yang perlu dicatat.";
    msg.className   = saved > 0 ? "form-msg success" : "form-msg";
    msg.style.background = saved > 0 ? "" : "#f1f5f9";
    msg.style.color      = saved > 0 ? "" : "#64748b";
    msg.style.border     = saved > 0 ? "" : "1px solid #e2e8f0";
    setTimeout(() => {
      msg.className = "form-msg hidden";
      msg.style = "";
    }, 3000);
    ...
    btn.textContent = "Simpan Update Capaian";
    btn.disabled    = false;
  }
});
