...
function switchTab(tab) {
  ...
  if (tab === "users" && can("manage_users")) loadUsers();
}
...
let allUsers = []; // cache {profile_id, user_id, name, role, avatar_initial, email, created_at}
...
async function loadUsers() {
  ...
  // Fetch semua profil + join dengan auth via Edge Function workaround:
  // Gunakan admin service jika tersedia, atau fallback ke user_profiles saja
...
function renderUsersTable(users) {
  ...
  tbody.innerHTML = users.map(u => {
    ...
    return `<tr>
      ...
      <td>
        <span class="role-pill role-${u.role}">${roleLbl}</span>
      </td>
...
document.getElementById('submitAddUserBtn').addEventListener('click', async () => {
  ...
  btn.disabled = true; spin.classList.remove('hidden'); btnTx.style.display = 'none';
  errEl.classList.add('hidden');

  // Buat user via Supabase Admin API (butuh service_role key di Edge Function)
  // Fallback: gunakan signUp lalu set profile manual
  ...
  if (authErr) {
    ...
    if (se) {
      errEl.textContent = se.message; errEl.classList.remove('hidden');
      btn.disabled = false; spin.classList.add('hidden'); btnTx.style.display = '';
      return;
    }
    if (sd?.user) await upsertProfile(sd.user.id, name, role, email);
  } else if (authData?.user) {
    await upsertProfile(authData.user.id, name, role, email);
  }
...
async function upsertProfile(userId, name, role, email) {
  const initial = name[0].toUpperCase();
  await client.from('user_profiles').upsert({
    user_id: userId, name, role, avatar_initial: initial, email
  }, { onConflict: 'user_id' });
}

// ── Edit Role Modal ─────────────────────────────────────────
function openEditRoleModal(profileId, name, currentRole) {
  document.getElementById('editRoleUserId').value        = profileId;
...
document.getElementById('submitEditRoleBtn').addEventListener('click', async () => {
  ...
  btn.disabled = false; btn.textContent = 'Simpan Role';
  document.getElementById('editRoleModal').classList.add('hidden');

  if (error) { showToast('Gagal mengubah role: ' + error.message, 'error'); return; }
...
async function confirmDeleteUser(profileId, name) {
  ...
  if (error) { showToast('Gagal menghapus user: ' + error.message, 'error'); return; }
