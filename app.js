...
document.getElementById("submitAllBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    const validOutcomes = outcomes.filter(oc => oc.text.trim());
    if (validOutcomes.length) {
      await client.from("project_outcomes").insert(
        validOutcomes.map((oc, i) => ({ project_name: p.name, outcome_text: oc.text.trim(), sort_order: i }))
      );
    }
      ...
      <div class="proj-card ${cls}" onclick="openProjectDetail(window.allProjects[${i}])">
        ...
        <div class="proj-card-budget">
          ...
          <div style="border-top:1px solid #f1f5f9;margin:8px 0 6px;padding-top:8px">
            ${item.goal ? `<div style="font-size:11px;color:#475569;margin-bottom:4px;line-height:1.4"><span style="font-weight:700;color:#2563eb">🎯 Goal:</span> ${item.goal}</div>` : ""}
            ${item.project_outcomes && item.project_outcomes.length ? `
              <div style="font-size:11px;color:#475569">
                ...
                <ul style="margin:3px 0 0 14px;padding:0;line-height:1.5">
                  ${item.project_outcomes.map(o => `<li style="margin-bottom:1px">${o.outcome_text}</li>`).join("")}
                </ul>
              </div>
            ` : ""}
          </div>
