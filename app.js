...
// Urutkan items sesuai order tersimpan; proyek baru (belum ada) masuk di akhir
...
// Reset urutan ke default (created_at)
...
// Buka project detail berdasarkan nama (lebih aman daripada index)
...
window.onCardDragStart = function(e, name) {
  _dragSrcName = name;
  e.dataTransfer.effectAllowed = "move";
  e.dataTransfer.setData("text/plain", name);
  setTimeout(() => {
    const el = document.querySelector(`.proj-card[data-name="${CSS.escape(name)}"]`);
...
window.onCardDrop = function(e, targetName) {
  e.preventDefault();
  ...
  // Ambil urutan saat ini dari DOM
  ...
  // Re-render cards saja (tanpa fetch ulang) agar responsif
...
window.onCardDragEnd = function(name) {
  document.querySelectorAll(".proj-card").forEach(c => {
    c.classList.remove("dragging");
    c.classList.remove("drag-over");
  });
...
function renderCards(items) {
  ...
  // Tambah tombol reset urutan jika ada order tersimpan
  const hasCustomOrder = getSortOrder().length > 0;
  const resetBtn = hasCustomOrder
    ? `<button class="btn-reset-order" onclick="resetCardOrder()" title="Kembalikan ke urutan awal">↺ Reset Urutan</button>`
    : "";
  const sortResetEl = document.getElementById("sortResetBtn");
  if (sortResetEl) sortResetEl.innerHTML = resetBtn;
  ...
  container.innerHTML = items.map((item, i) => {
    ...
    return `
      <div class="proj-card ${cls}"
        ...
        draggable="true"
        onclick="openProjectByName('${item.name.replace(/'/g, "\\\'")}')"
        ondragstart="onCardDragStart(event,'${item.name.replace(/'/g,"\\'")}')"
        ondragover="onCardDragOver(event,this)"
        ondragleave="onCardDragLeave(this)"
        ondrop="onCardDrop(event,'${item.name.replace(/'/g,"\\'")}')"
        ondragend="onCardDragEnd('${item.name.replace(/'/g,"\\'")}')">
...
function renderTable(items) {
  ...
  tbody.innerHTML = items.map((item, i) => {
    ...
    return `
      <tr style="cursor:pointer" onclick="openProjectByName('${item.name.replace(/'/g, "\\\'")}')">
        <td>${i+1}</td>
        ...
        <td>${dt}</td>
        <td>
          <button class="btn-edit" onclick="event.stopPropagation();fillFormEditByName('${item.name.replace(/'/g, "\\\'")}')">Edit</button>
