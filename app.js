...
// ===================== PRIORITY INDICATORS =====================
function renderPriorityIndicators(item) {
  const inds = item.project_indicators || [];
  if (!inds.length) return "";

  const lowInds = inds.map(ind => {
    const actual = getLatestActual(ind);
    const target = Number(ind.target) || 0;
    const pct    = target > 0 ? Math.min(Math.round((actual / target) * 100), 100) : 0;
    return { name: ind.indicator_name, actual, target, unit: ind.unit || "", pct };
  }).filter(x => x.pct < 50).sort((a, b) => a.pct - b.pct);

  if (!lowInds.length) return `
    <div class="priority-ind-section">
      <div class="priority-ind-title">
        <span>✅</span><span>Semua indikator &ge; 50%</span>
      </div>
    </div>`;
    ...
    <div class="priority-ind-section">
      ...
      <div class="priority-ind-list">
        ${shown.map(ind => {
          ...
          return `<div class="priority-ind-item" style="border-left:3px solid ${t.color};background:${t.bg}">
            <div class="priority-ind-left">
              <span class="priority-tier-dot" style="background:${t.color}"></span>
        ...
        ${rest > 0 ? `<div class="priority-ind-more">+${rest} indikator lainnya</div>` : ""}
      </div>
    </div>`;
}
...
// ===================== JUMP TO INDICATOR =====================
window.openProjectByName = function(name) {
  const proj = window.allProjects.find(p => p.name === name);
  if (proj) openProjectDetail(proj);
};

window.jumpToIndicator = async function(projName, indName, event) {
  if (event) event.stopPropagation();
  const proj = window.allProjects.find(p => p.name === projName);
  if (!proj) return;
...
// ===================== CARD SORT ORDER (localStorage) =====================
const SORT_KEY = "pmis_card_order";

function getSortOrder() {
  try { return JSON.parse(localStorage.getItem(SORT_KEY)) || []; }
  catch { return []; }
}
function saveSortOrder(names) {
  localStorage.setItem(SORT_KEY, JSON.stringify(names));
}
function applySortOrder(items) {
  const order = getSortOrder();
  if (!order.length) return items;
  const mapped = new Map(items.map(p => [p.name, p]));
  const sorted = order.map(name => mapped.get(name)).filter(Boolean);
  const rest   = items.filter(p => !order.includes(p.name));
  return [...sorted, ...rest];
}
...
window.onCardDragStart = function(e, el) {
  _dragSrcName = el.dataset.name;
  e.dataTransfer.effectAllowed = "move";
  e.dataTransfer.setData("text/plain", _dragSrcName);
  setTimeout(() => el.classList.add("dragging"), 0);
};

window.onCardDragOver = function(e, el) {
  e.preventDefault();
...
window.onCardDragLeave = function(el) {
  el.classList.remove("drag-over");
};

window.onCardDrop = function(e, el) {
  e.preventDefault();
  ...
  const cards = [...document.querySelectorAll(".proj-card[data-name]")];
  let names   = cards.map(c => c.dataset.name);
  const fromIdx = names.indexOf(_dragSrcName);
  const toIdx   = names.indexOf(targetName);
  if (fromIdx < 0 || toIdx < 0) return;
  names.splice(fromIdx, 1);
  names.splice(toIdx, 0, _dragSrcName);
  saveSortOrder(names);
  _dragSrcName = null;

  const reordered = names.map(n => window.allProjects.find(p => p.name === n)).filter(Boolean);
  window.allProjects = reordered;
  renderCards(reordered);
};

window.onCardDragEnd = function(el) {
...
async function loadProjects() {
  const { data: projects, error } = await client.from("projects").select().order("created_at", { ascending: true });
  ...
  const sortedItems = applySortOrder(items);
  window.allProjects = sortedItems;
  renderStats(sortedItems);
  renderCards(sortedItems);
  renderTable(sortedItems);
  renderSidebarSubmenu(sortedItems);

  if (currentProject && document.getElementById("tab-detail").classList.contains("active")) {
    const updated = sortedItems.find(p => p.name === currentProject.name);
...
function renderCards(items) {
  ...
  // Reset btn
  const sortResetEl = document.getElementById("sortResetBtn");
  if (sortResetEl) {
    sortResetEl.innerHTML = getSortOrder().length > 0
      ? `<button class="btn-reset-order" onclick="resetCardOrder()">↺ Reset Urutan</button>`
      : "";
  }
  ...
  container.innerHTML = items.map((item, i) => {
    ...
    return `
      <div class="proj-card ${cls}"
        data-name="${safeName}"
        draggable="true"
        onclick="openProjectByName(this.dataset.name)"
        ondragstart="onCardDragStart(event,this)"
        ondragover="onCardDragOver(event,this)"
        ondragleave="onCardDragLeave(this)"
        ondrop="onCardDrop(event,this)"
        ondragend="onCardDragEnd(this)">
        <div class="proj-card-header">
          <div class="proj-card-drag-handle" title="Seret untuk atur urutan" onclick="event.stopPropagation()"><svg width="10" height="16" viewBox="0 0 10 16" fill="none" xmlns="http://www.w3.org/2000/svg" style="display:block;pointer-events:none"><circle cx="2.5" cy="3" r="1.5" fill="currentColor"/><circle cx="7.5" cy="3" r="1.5" fill="currentColor"/><circle cx="2.5" cy="8" r="1.5" fill="currentColor"/><circle cx="7.5" cy="8" r="1.5" fill="currentColor"/><circle cx="2.5" cy="13" r="1.5" fill="currentColor"/><circle cx="7.5" cy="13" r="1.5" fill="currentColor"/></svg></div>
...
function renderTable(items) {
  ...
  tbody.innerHTML = items.map((item, i) => {
    ...
    return `
      <tr style="cursor:pointer" data-name="${item.name.replace(/"/g,'&quot;')}" onclick="openProjectByName(this.dataset.name)">
        <td>${i+1}</td>
        ...
        <td>${dt}</td>
        <td>
          <button class="btn-edit" onclick="event.stopPropagation();fillFormEditByName(this.closest('tr').dataset.name)">Edit</button>
...
// ===================== FILL FORM EDIT =====================
window.fillFormEditByName = function(name) {
  const idx = (window.allProjects || []).findIndex(p => p.name === name);
  if (idx >= 0) fillFormEdit(idx);
};
