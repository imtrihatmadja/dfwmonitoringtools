...
async function onLogin(user) {
  ...
  // Enforce super admin email always = role 'admin'
  if (isSuperAdmin(user.email)) {
    if (!profile || profile.role !== 'admin') {
      await client.from('user_profiles').upsert({
        user_id: user.id, email: user.email,
        name: profile?.name || 'Admin DFW',
    ...
    // Stamp email for all users (needed for user management)
...
function applyRoleGuards() {
  ...
  // Tab Kelola Pengguna: hanya admin
...
async function loadUsers() {
  ...
  if (!tbody) return;
  tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:#94a3b8;padding:24px">Memuat data pengguna...</td></tr>';
  const { data, error } = await client
    .from('user_profiles')
    .select('*')
    .order('created_at', { ascending: true });
  if (error) {
    tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;color:#dc2626;padding:24px">Gagal memuat: ${error.message}</td></tr>`;
    return;
  }
  allUsers = data || [];
...
function renderUsersTable(users) {
  ...
  if (!tbody) return;
  if (!users.length) {
    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:#94a3b8;padding:28px">Belum ada pengguna terdaftar.</td></tr>';
    return;
  }
  const colors = { admin:'#dc2626', manager:'#7c3aed', editor:'#2563eb', viewer:'#64748b' };
...
window.changeUserRole = async function(profileId, newRole, sel) {
  ...
  if (isSuperAdmin(u.email)) {
    showToast('Role Super Admin tidak dapat diubah.', 'error');
...
async function saveUser() {
  ...
  // Guard: cannot change super admin email or role
  if (uid) {
    const existing = allUsers.find(x => x.id === uid);
    if (existing && isSuperAdmin(existing.email)) {
      if (email !== existing.email) {
        showUserModalMsg('Email Super Admin tidak dapat diubah.', 'error'); return;
      }
      if (role !== existing.role) {
        showUserModalMsg('Role Super Admin tidak dapat diubah.', 'error'); return;
      }
    }
  }
  ...
  try {
    if (uid) {
      ...
      // ADD NEW — look up auth user_id by email via RPC
      // The RPC function must exist in Supabase (see schema.sql)
      const { data: authUserId, error: rpcErr } = await client.rpc('get_user_id_by_email', { p_email: email });
      if (rpcErr || !authUserId) {
        throw new Error('Email tidak ditemukan di Supabase Auth.\nDaftarkan terlebih dahulu:\nSupabase → Authentication → Users → Invite User');
      }
      // Check if profile already exists
      const { data: exists } = await client.from('user_profiles').select('id').eq('user_id', authUserId).maybeSingle();
      if (exists) throw new Error('Pengguna dengan email ini sudah terdaftar dalam sistem.');
    ...
    showUserModalMsg(err.message, 'error');
  } finally {
...
window.deleteUser = async function(profileId, userName) {
  ...
  if (currentProfile && currentProfile.id === profileId) {
    showToast('Anda tidak dapat menghapus akun sendiri.', 'error'); return;
  }
  ...
  if (error) {
    ...
    showToast(`Pengguna "${userName}" berhasil dihapus.`, 'success');
    await loadUsers();
  }
};
