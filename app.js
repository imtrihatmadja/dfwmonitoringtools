...
function renderIndicatorList() {
  ...
  container.innerHTML = indicators.map((ind, i) => `
    <div class="indicator-block">
      ...
      <div class="indicator-input-row" style="margin-top:8px">
        <div class="form-group">
          <label>Capaian Aktual (opsional)</label>
          <input type="number" id="actual-${i}" value="${ind.actual||""}" placeholder="0"
            oninput="indicators[${i}].actual=Number(this.value)" />
        </div>
        <div class="form-group" style="flex:2">
          <label>📝 Catatan Capaian Awal</label>
          <input type="text" id="ind-note-${i}" value="${escHtml(ind.update_note||'')}" placeholder="Catatan..."
...
document.getElementById("submitAllBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    const p = {
      ...
      budget_approved: Number(document.getElementById("f-budget-approved").value) || null,
      budget_actual:   Number(document.getElementById("f-budget-actual").value) || null
    };
...
(function() {
  ...
  // With defer, DOM is already ready - execute directly (no DOMContentLoaded needed)
