...
document.getElementById("submitAllBtn").addEventListener("click", async () => {
  ...
  try {
    const p = {
      ...
      budget_real: Number(document.getElementById("f-budget-real").value) || null,
    ...
    for (let i = 0; i < indicators.length; i++) {
      ...
      await client.from("project_indicators").insert({
        project_name: p.name, indicator_name: ind.name, type: ind.type,
        target: Number(ind.target)||0, unit: ind.unit||null, actual: 0
      });
    }
    msg.textContent = "✅ Proyek & indikator berhasil disimpan!";
...
function renderDetailHeader(proj) {
  ...
  const avgInd = inds.length
    ? Math.round(
        inds.reduce((a, ind) => {
          const pct = ind.target > 0 ? Math.min(Math.round((ind.actual / ind.target) * 100), 100) : 0;
          return a + pct;
        }, 0) / inds.length
      )
    : 0;
...
function renderIndicatorUpdatePanel(proj) {
  ...
  if (!inds.length) {
    container.innerHTML = `<div class="empty-state" style="padding:20px">Belum ada indikator.<br><small>Edit proyek untuk menambah indikator.</small></div>`;
    return;
  }

  container.innerHTML = inds.map((ind, i) => {
    const pct = ind.target > 0 ? Math.min(Math.round((ind.actual/ind.target)*100),100) : 0;
    const lastH = ind.indicator_updates && ind.indicator_updates.length
      ? ind.indicator_updates[ind.indicator_updates.length-1] : null;
    return `
      <div class="ind-update-card">
        <div class="ind-update-header">
          <div>
            <div class="ind-update-name">${ind.indicator_name}</div>
            <div class="ind-update-target">Target: <strong>${ind.target} ${ind.unit||""}</strong> &nbsp;|&nbsp; Capaian: <strong>${ind.actual} ${ind.unit||""}</strong></div>
          </div>
          <span class="badge badge-${(ind.type||"output").toLowerCase()}">${ind.type}</span>
        </div>
        <div class="progress-bar" style="margin-bottom:10px"><div class="progress-fill" style="width:${pct}%"></div></div>
        <div style="font-size:11px;color:#64748b;margin-bottom:8px">
          ${pct}% tercapai
          ${lastH ? ` &nbsp;|&nbsp; Update terakhir: ${new Date(lastH.created_at).toLocaleDateString("id-ID",{day:"2-digit",month:"short",year:"numeric"})}` : ""}
        </div>
        <div class="ind-update-inputs">
          <div class="form-group">
            <label>Capaian Baru</label>
            <input type="number" id="upd-actual-${i}" value="${ind.actual||""}" placeholder="0"
              oninput="updateIndPct(${i}, ${ind.target})" data-ind-id="${ind.id}" data-ind-name="${escHtml(ind.indicator_name)}" data-unit="${escHtml(ind.unit||"")}" />
          </div>
          <div class="form-group">
            <label>% Capaian</label>
            <input type="number" id="upd-pct-${i}" value="${pct||""}" readonly style="background:#f8fafc" />
          </div>
        </div>
        <div class="form-group" style="margin-top:6px">
          <label>📝 Catatan Update</label>
          <textarea id="upd-note-${i}" rows="2" placeholder="Tuliskan perkembangan, kendala, atau temuan lapangan..." style="font-size:12px">${""}</textarea>
        </div>
        ${ind.indicator_updates && ind.indicator_updates.length ? `
        <div class="mini-history" style="margin-top:8px">
          <div class="mini-history-title" style="display:flex;justify-content:space-between;align-items:center">
            <span>🕐 ${ind.indicator_updates.length} Riwayat Update</span>
            <button class="btn-danger btn-sm" style="font-size:10px;padding:3px 8px" onclick="clearIndicatorHistory('${ind.id}')">🗑️ Hapus Semua</button>
          </div>
          <div class="mini-history-list">
            ${ind.indicator_updates.slice().reverse().slice(0,3).map(h => `
              <div class="mini-history-item">
                <span class="mini-history-val">${h.actual_value} ${ind.unit||""}</span>
                <span class="mini-history-note">${h.note ? "— "+h.note : ""}</span>
                <span class="mini-history-date">${new Date(h.created_at).toLocaleDateString("id-ID",{day:"2-digit",month:"short",year:"numeric"})}</span>
              </div>`).join("")}
            ${ind.indicator_updates.length > 3 ? `<div class="mini-history-more">+${ind.indicator_updates.length-3} lainnya</div>` : ""}
          </div>
        </div>` : ""}
      </div>`;
  }).join("");
...
// ---- Simpan update capaian dari panel kanan ----
document.getElementById("saveIndicatorUpdatesBtn").addEventListener("click", async () => {
  if (!currentProject) return;
  const msg = document.getElementById("indUpdateMsg");
  const btn = document.getElementById("saveIndicatorUpdatesBtn");
  msg.className = "form-msg hidden";
  btn.textContent = "⏳ Menyimpan..."; btn.disabled = true;

  const inds = currentProject.project_indicators || [];
  let saved = 0;
  try {
    for (let i = 0; i < inds.length; i++) {
      const ind      = inds[i];
      const actualEl = document.getElementById("upd-actual-"+i);
      const noteEl   = document.getElementById("upd-note-"+i);
      if (!actualEl) continue;

      const newActual  = Number(actualEl.value) || 0;
      const note       = noteEl ? noteEl.value.trim() || null : null;
      const prevActual = Number(ind.actual) || 0;

      // Hanya simpan jika nilai BERUBAH
      if (newActual === prevActual && !note) continue;

      await client.from("project_indicators").update({ actual: newActual }).eq("id", ind.id);

      // Catat riwayat HANYA untuk indikator yang diubah
      await client.from("indicator_updates").insert({
        indicator_id:   ind.id,
      ...
      // Update nilai lokal
      currentProject.project_indicators[i].actual = newActual;
      saved++;
    }

    if (saved === 0) {
      msg.textContent = "ℹ️ Tidak ada perubahan yang disimpan.";
      msg.className = "form-msg hidden";
      msg.style.cssText = "display:block;padding:10px 14px;border-radius:8px;font-size:13px;font-weight:600;background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe;margin-top:10px";
      setTimeout(() => { msg.removeAttribute("style"); msg.className="form-msg hidden"; }, 2500);
      return;
    }

    // Hitung ulang avg capaian dari data lokal yang sudah diupdate
    const updInds = currentProject.project_indicators;
    const avgInd  = updInds.length
      ? Math.round(updInds.reduce((a, ind) => {
          const pct = ind.target > 0 ? Math.min(Math.round((ind.actual / ind.target) * 100), 100) : 0;
          return a + pct;
        }, 0) / updInds.length)
      : 0;
    const indDone = updInds.filter(ind =>
      (ind.target > 0 ? Math.round((ind.actual / ind.target) * 100) : 0) >= 100
    ).length;

    // Refresh mini dashboard langsung tanpa reload halaman
    document.querySelectorAll(".detail-stat").forEach(el => {
      const lbl = el.querySelector(".detail-stat-label");
      const val = el.querySelector(".detail-stat-value");
      if (!lbl || !val) return;
      if (lbl.textContent.trim() === "Avg. Capaian")       val.textContent = avgInd + "%";
      if (lbl.textContent.trim() === "Indikator Tercapai") val.textContent = indDone + "/" + updInds.length;
      if (lbl.textContent.trim() === "Update Terakhir")    val.textContent = new Date().toLocaleDateString("id-ID",{day:"2-digit",month:"short",year:"numeric"});
    });

    // Sync ke _allProjects
    const projIdx = (window._allProjects || []).findIndex(p => p.name === currentProject.name);
    if (projIdx >= 0) window._allProjects[projIdx].project_indicators = currentProject.project_indicators;

    msg.textContent = `✅ ${saved} indikator berhasil diperbarui!`;
    msg.className = "form-msg success";
    ...
    btn.textContent = "💾 Simpan Update Capaian"; btn.disabled = false;
  }
});
