...
window.populateDetailLearningActivities = async function(){
  ...
  sel.innerHTML = '<option value="">— Pilih kegiatan (opsional) —</option>' + (data||[]).map(a => `<option value="${a.id}">${a.title || ''}</option>`).join('');
};
...
window.saveDetailReflection = async function(){
  ...
  if (!cp || !cp.id) {
    if (msg) { msg.textContent = 'Proyek aktif tidak ditemukan.'; msg.className = 'form-msg error'; msg.style.display='block'; }
    return;
  }
  ...
  try {
    const authRes = await (window.client || client).auth.getUser();
    ...
    const { error } = await (window.client || client)
      .from('refleksi')
    ...
    if (msg) { msg.textContent = 'Gagal simpan refleksi: ' + (err.message || err); msg.className = 'form-msg error'; msg.style.display='block'; }
  } finally {
