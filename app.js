...
async function onLogin(user) {
  ...
  // Fetch profil — pakai maybeSingle() agar tidak error jika belum ada
  ...
  if (!profile) {
    ...
    const { data: np, error: npErr } = await client
      .from('user_profiles')
      .insert({
        user_id        : user.id,
      ...
      .select()
      .maybeSingle();
    if (npErr) console.warn('Profile insert error:', npErr.message);
    currentProfile = np || { role: 'viewer', name: user.email.split('@')[0] };
  } else {
    currentProfile = profile;
  }

  console.log('[PMIS] Login:', user.email, '| Role:', currentProfile?.role);

  updateSidebarUser();
  hideLoginPage();

  // applyRoleGuards setelah DOM visible
  requestAnimationFrame(() => {
    applyRoleGuards();
    // Panggil lagi setelah 300ms sebagai safety net
    setTimeout(() => applyRoleGuards(), 300);
  });

  setTimeout(() => initUserManagement(), 200);
  loadProjects();
}
...
function updateSidebarUser() {
  ...
  if (el('sidebarName'))  el('sidebarName').textContent = currentProfile.name || '–';
  if (el('sidebarRole'))  el('sidebarRole').textContent = ROLES[currentProfile.role] || currentProfile.role;
}

function applyRoleGuards() {
  const role = currentProfile?.role || 'viewer';
  console.log('[PMIS] applyRoleGuards, role:', role);

  // Helper: tampilkan/sembunyikan dengan eksplisit
  const show = (el, visible) => {
    if (!el) return;
    el.style.display = visible ? 'flex' : 'none';
  };

  // Tombol Tambah Proyek — set eksplisit agar tidak fallback ke CSS
  document.querySelectorAll('[data-guard="create"]').forEach(el => {
    const tag = el.tagName.toLowerCase();
    el.style.display = can('create')
      ? (tag === 'button' || tag === 'a' ? 'inline-flex' : 'flex')
      : 'none';
  });

  // Tab input di nav sidebar
  const inputNav = document.querySelector('.nav-links li[data-tab="input"]');
  show(inputNav, can('create'));

  // Bottom nav tambah
  const inputBottomNav = document.querySelector('.bottom-nav-item[data-tab="input"]');
  if (inputBottomNav) inputBottomNav.style.display = can('create') ? '' : 'none';

  // Nav Kelola User — hanya admin
  const usersNav = document.getElementById('navUsersItem');
  show(usersNav, can('manage_users'));

  console.log('[PMIS] navUsersItem:', usersNav, '| visible:', can("manage_users"));
}
...
const tabTitles = {
  ...
  users     : ["Kelola Pengguna", "Manajemen akses dan role pengguna"]
};
...
// ===================== USER MANAGEMENT =====================
const ROLE_LABELS = { admin:'Admin', manager:'Manager', editor:'Editor', viewer:'Viewer' };
const ROLE_COLORS = { admin:'#dc2626', manager:'#7c3aed', editor:'#2563eb', viewer:'#64748b' };
let allUsers = [];
let userMgmtReady = false;

async function loadUsers() {
  const tbody = document.getElementById('usersTableBody');
  if (!tbody) return;
  tbody.innerHTML = '<tr><td colspan="5" class="users-loading">Memuat...</td></tr>';
  const { data: profiles, error } = await client
    .from('user_profiles')
    .select('*')
    .order('created_at', { ascending: true });
  if (error) {
    tbody.innerHTML = `<tr><td colspan="5" class="users-loading" style="color:#dc2626">Gagal memuat: ${error.message}</td></tr>`;
    return;
  }
  allUsers = profiles || [];
...
function renderUsersTable(users) {
  ...
  if (!tbody) return;
  if (!users.length) {
    tbody.innerHTML = '<tr><td colspan="5" class="users-loading">Belum ada pengguna terdaftar.</td></tr>';
    return;
  }
  tbody.innerHTML = users.map(u => {
    const initial = u.avatar_initial || (u.name||'?')[0].toUpperCase();
    const roleClr = ROLE_COLORS[u.role] || '#64748b';
    ...
    return `<tr>
      <td>
        <div class="ut-user-cell">
          <div class="ut-avatar" style="background:${roleClr}">${initial}</div>
          <span class="ut-name">${escHtml(u.name||'–')}${isSelf?` <span class="ut-self-badge">Saya</span>`:''}</span>
        </div>
      </td>
      <td class="ut-email">${escHtml(u.email||'–')}</td>
      <td><span class="role-pill role-${u.role}">${roleLbl}</span></td>
      <td class="ut-date">${joinDate}</td>
      <td>
        <div class="ut-actions">
          <button class="ut-btn ut-btn-edit"
            onclick="openEditRoleModal('${u.id}','${escHtml(u.name||'')}','${u.role}')">✏️ Role</button>
          ${!isSelf ? `<button class="ut-btn ut-btn-del"
            onclick="confirmDeleteUser('${u.id}','${escHtml(u.name||'')}')">🗑</button>` : ''}
        </div>
      </td>
    </tr>`;
  }).join('');
...
function openAddUserModal() {
  ...
  f('newUserName'); f('newUserEmail'); f('newUserPassword'); f('newUserRole','viewer');
...
async function submitAddUser() {
  ...
  const errEl = document.getElementById('addUserError');
  const spin  = document.getElementById('addUserSpinner');
  const btnTx = document.getElementById('addUserBtnText');
  ...
  if (!pw || pw.length<6) { showErr('Password minimal 6 karakter.'); return; }
  ...
  // Daftar via signUp (tidak logout user saat ini karena pakai instance terpisah)
  const { data: sd, error: se } = await client.auth.signUp({ email, password: pw });
  if (se) {
    showErr(se.message);
  ...
  // Simpan profil ke user_profiles
  if (sd?.user) {
    const initial = name[0].toUpperCase();
    const { error: pe } = await client.from('user_profiles').upsert({
      user_id: sd.user.id, name, role, avatar_initial: initial, email
    }, { onConflict: 'user_id' });
...
function openEditRoleModal(profileId, name, currentRole) {
  const idEl   = document.getElementById('editRoleUserId');
  const nameEl = document.getElementById('editRoleUserName');
  const selEl  = document.getElementById('editRoleSelect');
  const modal  = document.getElementById('editRoleModal');
  if(idEl)   idEl.value         = profileId;
  if(nameEl) nameEl.textContent = name;
  if(selEl)  selEl.value        = currentRole;
  if(modal)  modal.classList.remove('hidden');
}
...
async function submitEditRole() {
  ...
  const btn  = document.getElementById('submitEditRoleBtn');
  if (!id || !role) return;
  if(btn) { btn.disabled=true; btn.textContent='Menyimpan...'; }
  const { error } = await client.from('user_profiles').update({ role }).eq('id', id);
  if(btn) { btn.disabled=false; btn.textContent='Simpan Role'; }
  closeEditRoleModal();
...
async function confirmDeleteUser(profileId, name) {
  if (!confirm(`Hapus akses pengguna "${name}"?\nUser tidak bisa login setelah ini.`)) return;
  ...
  showToast(`🗑 Akses ${name} dicabut`,'info');
  loadUsers();
}
...
// Setup event listener modal — dipanggil setelah DOM siap
function initUserManagement() {
  if (userMgmtReady) return;
  userMgmtReady = true;
  ...
  // Tutup modal tambah
  ['closeAddUserModal','cancelAddUserBtn'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.addEventListener('click', closeAddUserModal);
  });

  // Submit tambah user
  const subBtn = document.getElementById('submitAddUserBtn');
  if (subBtn) subBtn.addEventListener('click', submitAddUser);

  // Tutup modal edit role
  ['closeEditRoleModal','cancelEditRoleBtn'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.addEventListener('click', closeEditRoleModal);
  });

  // Submit edit role
  const subRoleBtn = document.getElementById('submitEditRoleBtn');
  if (subRoleBtn) subRoleBtn.addEventListener('click', submitEditRole);

  // Tutup modal saat klik overlay
  ['addUserModal','editRoleModal'].forEach(id => {
    const el = document.getElementById(id);
  ...
  // Enter key di form tambah user
  ['newUserName','newUserEmail','newUserPassword'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.addEventListener('keydown', e => { if(e.key==='Enter') submitAddUser(); });
  });
}
