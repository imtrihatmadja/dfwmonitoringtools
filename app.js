...
function renderDetailHeader(proj) {
  ...
  const indBawah50 = inds.reduce((acc, ind, idx) => {
    const actual = ind.indicator_updates && ind.indicator_updates.length
    ...
    if (pct < 50) {
      acc.push({ name: ind.indicator_name, actual, target: ind.target, unit: ind.unit||"", pct, idx });
    }
    return acc;
  }, []);
  ...
  document.getElementById("detailHeader").innerHTML = `
    ...
    <div class="ind-alert-box">
      ...
      <div class="ind-alert-list">
        ${indBawah50.map(ind => {
          const barColor = ind.pct >= 35 ? "#f59e0b" : "#ef4444";
          return `<div class="ind-alert-item" onclick="scrollToIndicator(${ind.idx})" title="Klik untuk update indikator ini">
...
window.scrollToIndicator = function(idx) {
  ...
  // Scroll panel kanan ke kartu yang dituju
  const cardTop    = card.offsetTop - scrollArea.offsetTop;
  scrollArea.scrollTo({ top: cardTop - 12, behavior: "smooth" });

  // Highlight animasi (pulse border + background)
  card.classList.add("ind-card-highlight");
  setTimeout(() => card.classList.remove("ind-card-highlight"), 2200);

  // Auto-fokus ke input tambahan nilai
  setTimeout(() => {
