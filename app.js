...
async function initAuth() {
  ...
  // Pasang listener DULU sebelum getSession
  client.auth.onAuthStateChange(async (event, session) => {
    if (event === 'SIGNED_IN' && session) {
      if (_loginInProgress) return; // cegah double-call
      try {
        await onLogin(session.user);
      } catch(err) {
        // FIX: Reset flag jika onLogin gagal agar retry bisa berjalan
        _loginInProgress = false;
        console.error('onAuthStateChange onLogin error:', err);
      }
    }
    if (event === 'SIGNED_OUT') {
      _loginInProgress = false;
...
async function onLogin(user) {
  ...
  try {
    ...
    let { data: profile, error: fetchErr } = await client
      ...
      .select('*')
      .eq('user_id', user.id)
      .maybeSingle();   // maybeSingle() tidak throw jika tidak ada hasil

    if (fetchErr) console.warn('Fetch profile warning:', fetchErr.message);

    if (!profile) {
      // Auto-create profil untuk user baru
      const initial = (user.email || '?')[0].toUpperCase();
      const name    = user.email.split('@')[0];
      const role    = isSuperAdmin(user.email) ? 'admin' : 'viewer';
      const { data: np, error: insErr } = await client
        .from('user_profiles')
        .insert({ user_id: user.id, name, role, avatar_initial: initial })
        .select()
        .maybeSingle();
      if (insErr) console.warn('Create profile warning:', insErr.message);
      profile = np;
    }
    ...
    if (isSuperAdmin(user.email) && profile && profile.role !== 'admin') {
      const { error: updErr } = await client
        ...
        .update({ role: 'admin' })
        .eq('user_id', user.id);
      if (!updErr) profile = { ...profile, role: 'admin' };
    }

    // Jika profil masih null (DB error), buat objek fallback agar app tetap bisa jalan
    if (!profile) {
      profile = {
        user_id        : user.id,
        name           : user.email.split('@')[0],
        role           : isSuperAdmin(user.email) ? 'admin' : 'viewer',
        avatar_initial : (user.email || '?')[0].toUpperCase(),
      };
    }

    // Gabungkan email dari auth session (tidak disimpan ke DB)
    ...
    // Fallback darurat: tetap masuk dengan profil minimal
    currentProfile = {
      user_id        : user.id,
      name           : user.email.split('@')[0],
      email          : user.email,
...
// Login error helper (must be defined before setupLogin IIFE)
...
// Login handler
(function setupLogin() {
  // FIX: Guard null check — jika elemen login belum ada, skip
  const loginBtnEl = document.getElementById('loginBtn');
  if (!loginBtnEl) { console.warn('setupLogin: #loginBtn tidak ditemukan'); return; }
  ...
  loginBtnEl.addEventListener('click', async () => {
    ...
    // FIX: Wrap dengan try/catch untuk menangkap network error
    try {
      ...
      showLoginError('Koneksi gagal. Periksa koneksi internet Anda.');
...
document.getElementById("submitAllBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    // Deteksi mode: edit (proyek sudah ada) vs tambah baru
    const prevProj = (window.allProjects || []).find(x => x.name === p.name);
    const isEdit   = !!prevProj;
    const prevBudgetActual = prevProj ? (prevProj.budget_actual || 0) : -1;

    // STEP 1: Upsert proyek + simpan outcomes secara paralel
    const validOutcomes = outcomes.filter(oc => oc.text && oc.text.trim());
    const [projResult] = await Promise.all([
      client.from("projects").upsert(p, { onConflict: "name" }),
      // Outcomes: hapus lama lalu insert baru (serentak dengan upsert proyek)
      client.from("project_outcomes").delete().eq("project_name", p.name),
    ]);
    if (projResult.error) throw new Error("Gagal simpan proyek: " + projResult.error.message);

    // Insert outcomes baru (setelah delete selesai)
    if (validOutcomes.length) {
      const { error: ocErr } = await client.from("project_outcomes").insert(
        validOutcomes.map((oc, oidx) => ({ project_name: p.name, outcome_text: oc.text.trim(), sort_order: oidx }))
      );
      if (ocErr) console.warn("Outcomes warning:", ocErr.message);
    }

    // STEP 2: Catat histori budget_actual jika berubah
    if (p.budget_actual > 0 && p.budget_actual !== prevBudgetActual) {
      await client.from("budget_updates").insert({
        project_name : p.name,
        actual_value : p.budget_actual,
        note         : null,
        updated_by   : currentProfile?.name || "Tim",
      });
    }

    // STEP 3: Simpan indikator
    // FIX BUG KRITIS: Jangan hapus semua indikator saat EDIT karena akan menghapus
    // seluruh riwayat indicator_updates (ON DELETE CASCADE)!
    // Strategi:
    //   - EDIT: upsert indikator (preserve id & riwayat), hanya hapus yang dihapus user
    //   - BARU: insert langsung (tidak ada riwayat sebelumnya)

    const validInds = indicators.filter(ind => ind.name && ind.name.trim());

    if (isEdit) {
      // === MODE EDIT: Preserve riwayat indikator ===
      // Fetch indikator yang ada di DB saat ini
      const { data: dbInds } = await client
        .from("project_indicators")
        .select("id, indicator_name")
        .eq("project_name", p.name);

      const dbIndMap = {};
      (dbInds || []).forEach(d => { dbIndMap[d.indicator_name] = d.id; });

      // Indikator yang sudah tidak ada di form → hapus
      const keepNames = new Set(validInds.map(ind => ind.name.trim()));
      const toDelete  = (dbInds || []).filter(d => !keepNames.has(d.indicator_name));
      if (toDelete.length) {
        await client.from("project_indicators")
          .delete()
          .in("id", toDelete.map(d => d.id));
      }

      // Upsert indikator yang ada di form secara paralel (Promise.all)
      const upsertResults = await Promise.all(
        validInds.map(async (ind) => {
          const existingId = ind.id || dbIndMap[ind.name.trim()] || null;
          if (existingId) {
            // UPDATE indikator yang sudah ada (preserve id, preserve riwayat)
            const { error: updErr } = await client
              .from("project_indicators")
              .update({
              ...
              .eq("id", existingId);
            if (updErr) { console.warn("Update ind error:", updErr.message); return null; }
            return { id: existingId, ind };
          } else {
            // INSERT indikator baru yang ditambahkan saat edit
            const { data: newInd, error: insErr } = await client
              .from("project_indicators")
              .insert({
                project_name   : p.name,
                indicator_name : ind.name.trim(),
              ...
              .select("id").single();
            if (insErr) { console.warn("Insert new ind error:", insErr.message); return null; }
            return { id: newInd.id, ind, isNew: true };
          }
        })
      );

      // Insert indicator_update hanya untuk indikator BARU yang punya nilai actual
      const newUpdInserts = upsertResults
        .filter(r => r && r.isNew && (r.ind.actual > 0 || r.ind.update_note))
        .map(r => ({
          indicator_id   : r.id,
          project_name   : p.name,
          indicator_name : r.ind.name.trim(),
          actual_value   : r.ind.actual || 0,
          note           : r.ind.update_note || null,
          updated_by     : currentProfile?.name || "Tim",
        }));
      if (newUpdInserts.length) {
        await client.from("indicator_updates").insert(newUpdInserts);
      }

      // Sync id ke array indicators
      upsertResults.forEach((r, i) => { if (r) indicators[i] && (indicators[i].id = r.id); });

    } else {
      // === MODE TAMBAH BARU: Insert semua indikator secara paralel ===
      const insertResults = await Promise.all(
        validInds.map(ind =>
          client.from("project_indicators").insert({
            project_name   : p.name,
            indicator_name : ind.name.trim(),
            type           : ind.type,
            target         : Number(ind.target) || 0,
      ...
      // Insert indicator_updates untuk yang punya nilai awal (paralel)
      const initUpdInserts = [];
      insertResults.forEach((res, i) => {
        if (res.error) { console.warn("Insert ind error:", res.error.message); return; }
        const ind = validInds[i];
        indicators[i] && (indicators[i].id = res.data.id);
        if (ind.actual > 0 || ind.update_note) {
          initUpdInserts.push({
            indicator_id   : res.data.id,
            project_name   : p.name,
            indicator_name : ind.name.trim(),
      ...
      if (initUpdInserts.length) {
        await client.from("indicator_updates").insert(initUpdInserts);
      }
    }
...
window.saveOneIndicator = async function (i, indId, currentActual, target, indName, unit) {
  ...
  try {
    ...
    // FIX: Jalankan update + insert PARALEL — bukan serial
    const [updRes, insRes] = await Promise.all([
      client.from("project_indicators").update({ actual: newTotal }).eq("id", indId),
      client.from("indicator_updates").insert(newEntry).select().single(),
    ]);

    if (updRes.error) throw new Error("Gagal update indikator: " + updRes.error.message);
    if (insRes.error) throw new Error("Gagal catat riwayat: " + insRes.error.message);

    // Kosongkan input
    addEl.value = "";
    if (noteEl) noteEl.value = "";

    // Sukses ditampilkan SEGERA — tidak tunggu reload
    msgEl.textContent   = `✅ Capaian diperbarui: ${currentActual} + ${addVal} = ${newTotal} ${unit}`;
    msgEl.className     = "form-msg success";
    msgEl.style.display = "block";

    // FIX: Update in-memory langsung (tanpa reload semua data dari server)
    if (currentProject && window.allProjects) {
      const projIdx = window.allProjects.findIndex(p => p.name === currentProject.name);
      if (projIdx !== -1) {
        const proj  = window.allProjects[projIdx];
        const indIdx = proj.project_indicators.findIndex(ind => ind.id === indId);
        if (indIdx !== -1) {
          const ind = proj.project_indicators[indIdx];
          // Tambah entry baru ke riwayat in-memory
          const newHistEntry = insRes.data || {
            id: Date.now(), indicator_id: indId, actual_value: newTotal,
            note, updated_by: currentProfile?.name || "Tim",
            created_at: new Date().toISOString(),
          };
          proj.project_indicators[indIdx] = {
            ...ind,
            actual: newTotal,
            indicator_updates: [...(ind.indicator_updates || []), newHistEntry],
          };
          currentProject = proj;
          window.allProjects[projIdx] = proj;
        }
      }
      // Re-render hanya panel detail — tidak reload seluruh dashboard
      renderDetailHeader(currentProject);
      renderIndicatorUpdatePanel(currentProject);
    }

    // Background refresh ringan: hanya update stats dashboard tanpa block UI
    setTimeout(async () => {
      try {
        const { data: updProj } = await client.from("projects")
          .select("progress, updated_at").eq("name", currentProject?.name || "").single();
        if (updProj && window.allProjects) {
          const idx = window.allProjects.findIndex(p => p.name === currentProject?.name);
          if (idx !== -1) {
            window.allProjects[idx].progress     = updProj.progress;
            window.allProjects[idx].updated_at   = updProj.updated_at;
          }
          renderStats(window.allProjects);
        }
      } catch (_) { /* silent — background only */ }
    }, 500);
    ...
    msgEl.className     = "form-msg error";
    msgEl.style.display = "block";
    console.error("saveOneIndicator error:", err);
  } finally {
...
window.toggleActDone = async function (id, wasChecked) {
  ...
  // FIX: Jalankan update DB + reload aktivitas paralel, tidak perlu full loadProjects
  const [, ] = await Promise.all([
    client.from("project_activities").update(update).eq("id", id),
    loadActivities(currentActProject),
  ]);
  // Update in-memory allProjects untuk statistik aktivitas
  if (window.allProjects) {
    const proj = window.allProjects.find(p => p.name === currentActProject);
    if (proj) {
      const act = (proj.activities_summary || []).find(a => a.id === id);
      if (act) { act.status = update.status; if (update.progress) act.progress = update.progress; }
    }
    renderStats(window.allProjects);
  }
};
...
window.saveInlineNote = async function (actId) {
  ...
  // FIX: Insert + clear input segera, reload hanya aktivitas (bukan full loadProjects)
  const { error } = await client.from("activity_notes").insert({
    activity_id  : actId,
    project_name : currentActProject,
    note,
    noted_by     : currentProfile?.name || "Tim",
  });
  if (error) { alert("Gagal menyimpan catatan: " + error.message); return; }
  ta.value = "";
...
window.deleteActivity = async function (id) {
  ...
  // FIX: Paralel delete + reload aktivitas, tidak perlu full loadProjects
  await Promise.all([
    client.from("project_activities").delete().eq("id", id),
    Promise.resolve(), // placeholder agar Promise.all simetris
  ]);
  await loadActivities(currentActProject);
  // Update in-memory
  if (window.allProjects) {
    const proj = window.allProjects.find(p => p.name === currentActProject);
    if (proj) {
      proj.activities_summary = (proj.activities_summary || []).filter(a => a.id !== id);
      proj.activityCount = (proj.activityCount || 1) - 1;
    }
    renderStats(window.allProjects);
  }
};
...
window.clearIndicatorHistory = async function (indicatorId) {
  ...
  // FIX: Update in-memory langsung, tidak perlu full loadProjects
  if (currentProject && window.allProjects) {
    const proj = window.allProjects.find(p => p.name === currentProject.name);
    if (proj) {
      const ind = proj.project_indicators.find(ind => ind.id === indicatorId);
      if (ind) ind.indicator_updates = [];
      currentProject = proj;
    }
    renderDetailHeader(currentProject);
    renderIndicatorUpdatePanel(currentProject);
