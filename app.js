...
async function onLogin(user) {
  ...
  applyRoleGuards();
  hideLoginPage();
  // Init user management setelah login (DOM sudah tersedia)
...
function applyRoleGuards() {
  ...
  // Nav users — hanya admin (gunakan id untuk reliability)
  const usersNav = document.getElementById('navUsersItem')
    || document.querySelector('[data-tab="users"]');
  if (usersNav) usersNav.style.display = can('manage_users') ? '' : 'none';
  // Semua elemen data-guard="manage_users"
  document.querySelectorAll('[data-guard="manage_users"]').forEach(el => {
    el.style.display = can('manage_users') ? '' : 'none';
  });
}
