...
document.getElementById("toStep3Btn").addEventListener("click", () => {
  if (!indicators.length) { alert("Harap tambahkan minimal 1 indikator."); return; }
  setStep(3);
...
document.getElementById("backStep2Btn").addEventListener("click", () => { setStep(2); renderIndicatorList(); });
...
document.getElementById("submitAllBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    const _budgApprovedInput = Number(document.getElementById("f-budget-approved")?.value) || null;
    const _budgActualInput   = Number(document.getElementById("f-budget-actual")?.value)   || null;
    ...
    const p = {
      ...
      budget_approved: _budgApprovedInput,
      budget_actual:   _budgActualInput
    };
    ...
    // Simpan histori realisasi anggaran jika diisi
    if (_budgActualInput) {
      await client.from("budget_updates").insert({
        project_name: p.name, actual_value: _budgActualInput,
        note: "Input via form proyek", updated_by: "Tim"
      });
    }
...
function resetForm() {
  ["f-name","f-location","f-owner","f-donor","f-start-date","f-deadline","f-desc","f-note","f-budget-approved","f-budget-actual"].forEach(id => {
...
async function loadProjects() {
  ...
  const { data: budgUpds } = await client.from("budget_updates").select("*").order("created_at", { ascending: false });
  ...
  const items = (projects||[]).map(proj => ({
    ...
    budget_history: (budgUpds||[]).filter(b => b.project_name === proj.name)
  }));
...
function renderCards(items) {
  ...
  container.innerHTML = items.map((item, i) => {
    ...
    const budgApproved = Number(item.budget_approved) || 0;
    const budgActual   = Number(item.budget_actual)   || 0;
    const budgPct      = budgApproved > 0 ? Math.min(Math.round((budgActual / budgApproved) * 100), 200) : 0;
    const budgColor    = budgPct > 100 ? "#ef4444" : budgPct > 80 ? "#f59e0b" : "#22c55e";
    const lastBudg     = item.budget_history && item.budget_history.length ? item.budget_history[0] : null;
    ...
    return `
      <div class="proj-card ${cls}" onclick="openProjectDetail(window._allProjects[${i}])">
        ...
        <div class="proj-card-budget" onclick="event.stopPropagation()">
          <div class="proj-card-budget-row">
            <span style="font-weight:600;color:#92400e;font-size:11px">💰 Anggaran</span>
            <button class="btn-secondary btn-sm" style="font-size:10px;padding:2px 8px"
              onclick="event.stopPropagation();openBudgetModal('${safeN}',${budgActual},${budgApproved})">✏️ Update</button>
          </div>
          ${budgApproved > 0 ? `
            <div class="proj-card-budget-row" style="margin-top:4px">
              <span style="color:#64748b;font-size:11px">Disetujui: <strong style="color:#0f172a">Rp ${fmtRupiah(budgApproved)}</strong></span>
              <span style="color:${budgColor};font-weight:700;font-size:12px">Rp ${fmtRupiah(budgActual)} <small>(${budgPct}%)</small></span>
            </div>
            <div class="progress-bar" style="height:4px;margin-top:4px">
              <div class="progress-fill" style="width:${Math.min(budgPct, 100)}%;background:${budgColor}"></div>
            </div>
            ${lastBudg ? `<div style="font-size:10px;color:#94a3b8;margin-top:3px">Update: ${new Date(lastBudg.created_at).toLocaleDateString("id-ID",{day:"2-digit",month:"short",year:"numeric"})}</div>` : ""}
          ` : `<div style="font-size:11px;color:#94a3b8;font-style:italic;margin-top:3px">Anggaran belum diset</div>`}
        </div>
        <div class="proj-card-actions">
          ...
          <button class="btn-danger btn-sm" style="margin-left:auto"
            onclick="event.stopPropagation();deleteProject('${item.id}','${safeN}')">🗑️ Hapus</button>
        </div>
      </div>`;
  }).join("");
}
...
function renderDetailHeader(proj) {
  ...
  const indDone = inds.filter(ind => {
    ...
    return tgt > 0 && Math.round((getLastActual(ind) / tgt) * 100) >= 100;
  }).length;

  const cls = proj.status.toLowerCase().replace(/ /g, "-");

  const avgInd = inds.length
    ? Math.round(inds.reduce((acc, ind) => {
        const tgt = Number(ind.target) || 0;
        return acc + (tgt > 0 ? Math.min(Math.round((getLastActual(ind) / tgt) * 100), 100) : 0);
      }, 0) / inds.length)
    : null;

  const avgAct = allActivities.length
    ? Math.round(allActivities.reduce((a, b) => a + (b.progress || 0), 0) / allActivities.length)
    : null;

  let combined;
  if (avgAct !== null && avgInd !== null) combined = Math.round((avgAct + avgInd) / 2);
  else if (avgInd !== null) combined = avgInd;
  else if (avgAct !== null) combined = avgAct;
  else combined = proj.progress || 0;

  const progColor = combined >= 80 ? "#22c55e"
    : combined >= 60 ? "#3b82f6"
    : combined >= 40 ? "#f59e0b"
    : "#ef4444";

  const budgApproved = Number(proj.budget_approved) || 0;
  const budgActual   = Number(proj.budget_actual)   || 0;
  const budgPct      = budgApproved > 0 ? Math.min(Math.round((budgActual / budgApproved) * 100), 200) : 0;
  const budgColor    = budgPct > 100 ? "#ef4444" : budgPct > 80 ? "#f59e0b" : "#3b82f6";
  const lastBudg     = proj.budget_history && proj.budget_history.length ? proj.budget_history[0] : null;
  const safeProjName = (proj.name || "").replace(/'/g, "\\'");
  ...
  document.getElementById("detailHeader").innerHTML = `
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;flex-wrap:wrap;gap:6px">
      ...
      <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
        ${avgAct !== null ? `<span style="font-size:11px;background:#eff6ff;color:#2563eb;border-radius:20px;padding:2px 9px;border:1px solid #bfdbfe">🏃 Aktivitas: ${avgAct}%</span>` : ""}
        ${avgInd !== null ? `<span style="font-size:11px;background:#f0fdf4;color:#16a34a;border-radius:20px;padding:2px 9px;border:1px solid #bbf7d0">📊 Ind: ${avgInd}%</span>` : ""}
        <span style="font-size:24px;font-weight:800;color:${progColor};line-height:1">${combined}%</span>
      </div>
    </div>
    <div class="progress-bar" style="height:14px;border-radius:8px;margin-bottom:18px">
      <div class="progress-fill" style="width:${combined}%;background:${progColor};border-radius:8px;transition:width 0.6s ease"></div>
    </div>

    <div class="detail-stats">
      <div class="detail-stat">
        <div class="detail-stat-label">Total Indikator</div>
        <div class="detail-stat-value">${inds.length}</div>
      </div>
      <div class="detail-stat">
        <div class="detail-stat-label">Indikator Tercapai</div>
        <div class="detail-stat-value" style="color:#22c55e">${indDone} / ${inds.length}</div>
      </div>
      <div class="detail-stat" style="border-left:3px solid ${progColor}">
        <div class="detail-stat-label">Avg. Capaian Ind.</div>
        <div class="detail-stat-value" style="color:${progColor}">${avgInd !== null ? avgInd + "%" : "—"}</div>
      </div>
      <div class="detail-stat" style="border-left:3px solid #3b82f6">
        <div class="detail-stat-label">Avg. Progress Akt.</div>
        <div class="detail-stat-value" style="color:#3b82f6">${avgAct !== null ? avgAct + "%" : "—"}</div>
      </div>
    </div>

    ${budgApproved > 0 ? `
    <div style="margin-top:14px;background:#fffbeb;border:1px solid #fde68a;border-radius:10px;padding:12px 14px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
        <span style="font-size:11px;font-weight:700;color:#92400e;text-transform:uppercase;letter-spacing:.5px">💰 Realisasi Anggaran</span>
        <button class="btn-secondary btn-sm" style="font-size:11px;padding:3px 10px"
          onclick="openBudgetModal('${safeProjName}',${budgActual},${budgApproved})">✏️ Update</button>
      </div>
      <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:6px;flex-wrap:wrap;gap:4px">
        <span style="color:#64748b">Disetujui: <strong style="color:#0f172a">Rp ${fmtRupiah(budgApproved)}</strong></span>
        <span style="color:#64748b">Realisasi: <strong style="color:${budgColor}">Rp ${fmtRupiah(budgActual)}</strong>
          <span style="font-size:11px;background:${budgColor}22;color:${budgColor};border-radius:20px;padding:1px 7px;margin-left:4px;border:1px solid ${budgColor}44">${budgPct}%</span>
        </span>
      </div>
      <div class="progress-bar" style="height:6px;border-radius:4px">
        <div class="progress-fill" style="width:${Math.min(budgPct,100)}%;background:${budgColor};border-radius:4px"></div>
      </div>
      ${lastBudg ? `<div style="font-size:11px;color:#94a3b8;margin-top:5px">Diperbarui: ${new Date(lastBudg.created_at).toLocaleString("id-ID",{day:"2-digit",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"})}${lastBudg.note ? " — " + lastBudg.note : ""}</div>` : ""}
    </div>` : ""}
  `;
}
...
function renderIndicatorUpdatePanel(proj) {
  ...
  // Dedup indikator berdasarkan id
  const _seen = new Set();
  const inds = (proj.project_indicators || []).filter(ind => {
    if (_seen.has(ind.id)) return false;
    _seen.add(ind.id);
    return true;
  });
  if (!inds.length) {
  ...
  container.innerHTML = inds.map((ind, i) => {
    ...
    const safeName = (ind.indicator_name||"").replace(/'/g, "\\'");
    const safeUnit = (ind.unit||"").replace(/'/g, "\\'");

    return `<div class="ind-update-card">
      ...
      <!-- Riwayat -->
      ${sortedUpd.length ? `<div class="mini-history" style="margin-top:10px">
        <div class="mini-history-title" style="display:flex;justify-content:space-between;align-items:center">
          ...
          <button class="btn-danger btn-sm" style="font-size:10px;padding:3px 8px"
            onclick="clearIndicatorHistory('${safeId}')">Hapus Semua</button>
        </div>
        <div class="mini-history-list">
          ${[...sortedUpd].reverse().slice(0,5).map(h => `<div class="mini-history-item">
...
window.fillFormEdit = function(idx) {
  ...
  const _fba = document.getElementById("f-budget-approved");
  const _fbac = document.getElementById("f-budget-actual");
  if (_fba)  _fba.value  = item.budget_approved || "";
  if (_fbac) _fbac.value = item.budget_actual   || "";
...
async function loadActivities(projectName) {
  ...
  // Update progress keseluruhan setelah aktivitas dimuat
  if (currentProject) renderDetailHeader(currentProject);
}
...
// ===================== BUDGET MODAL =====================
window.openBudgetModal = function(projName, currentActual, budgetApproved) {
  currentBudgetProject = projName;
  const nameEl = document.getElementById("budg-proj-name");
  const appEl  = document.getElementById("budg-approved-val");
  const actEl  = document.getElementById("budg-actual-input");
  const noteEl = document.getElementById("budg-note-input");
  const msgEl  = document.getElementById("budgModalMsg");
  if (nameEl) nameEl.textContent = projName;
  if (appEl)  appEl.textContent  = "Rp " + fmtRupiah(budgetApproved);
  if (actEl)  actEl.value        = currentActual || "";
  if (noteEl) noteEl.value       = "";
  if (msgEl)  { msgEl.textContent = ""; msgEl.className = "form-msg hidden"; }
  const overlay = document.getElementById("budgModalOverlay");
  if (overlay) overlay.classList.remove("hidden");
};

["budgModalClose", "budgModalClose2"].forEach(id => {
  const el = document.getElementById(id);
...
const _saveBudgBtn = document.getElementById("saveBudgetBtn");
if (_saveBudgBtn) {
  _saveBudgBtn.addEventListener("click", async () => {
    const actEl  = document.getElementById("budg-actual-input");
    const noteEl = document.getElementById("budg-note-input");
    const msgEl  = document.getElementById("budgModalMsg");
    const newActual = Number(actEl?.value);
    const note      = noteEl?.value.trim() || null;
    if (!currentBudgetProject) return;
    if (isNaN(newActual) || newActual < 0) {
      if (msgEl) { msgEl.textContent = "⚠️ Masukkan nilai anggaran yang valid."; msgEl.className = "form-msg"; }
      return;
    }
    _saveBudgBtn.textContent = "Menyimpan…"; _saveBudgBtn.disabled = true;
    try {
      await client.from("projects").update({ budget_actual: newActual }).eq("name", currentBudgetProject);
      await client.from("budget_updates").insert({
        project_name: currentBudgetProject, actual_value: newActual,
        note, updated_by: "Tim"
      });
      const overlay = document.getElementById("budgModalOverlay");
      if (overlay) overlay.classList.add("hidden");
      ...
      _saveBudgBtn.textContent = "💾 Simpan"; _saveBudgBtn.disabled = false;
    }
  });
}
