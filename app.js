    ...
    return `
      <div class="proj-card ${cls}" onclick="openProjectDetail(window.allProjects[${i}])">
        ...
        <div class="proj-card-budget">
          ...
          <div style="border-top:1px solid #f1f5f9;margin:8px 0 6px;padding-top:8px;display:flex;flex-wrap:wrap;gap:4px;align-items:center">
            ${item.goal ? `<div style="font-size:10px;color:#475569;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%;display:flex;align-items:center;gap:4px;width:100%"><span style="font-weight:700;color:#2563eb;flex-shrink:0">🎯</span><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${item.goal}</span></div>` : ""}
            ${item.project_outcomes && item.project_outcomes.length ? `<span style="font-size:10px;background:#f5f3ff;color:#7c3aed;padding:2px 7px;border-radius:10px;font-weight:600">🏆 ${item.project_outcomes.length} Outcome${item.project_outcomes.length > 1 ? "s" : ""}</span>` : ""}
          </div>
        ` : ""}
