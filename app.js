    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px;margin-bottom:14px">
      <div>
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px">
          ...
          <button class="btn-primary btn-sm" onclick="openReportModal()" style="font-size:12px;background:#0f172a;border-color:#0f172a">&#128438; Cetak Laporan</button>
...
function openReportModal() {
  if (!currentProject) { showToast('Pilih proyek terlebih dahulu.', 'error'); return; }
...
window.generateReport = async function(lang) {
  ...
  // Load fresh activities + notes for this project
  const { data: acts  } = await client.from('project_activities')
    .select('*').eq('project_name', proj.name)
    .order('sort_order').order('created_at');
  const { data: notes } = await client.from('activity_notes')
    .select('*').eq('project_name', proj.name)
    .order('created_at', { ascending: false });
  const { data: indUpds } = await client.from('indicator_updates')
    .select('*').eq('project_name', proj.name)
    .order('created_at', { ascending: false });
  const { data: budgets } = await client.from('budget_updates')
    .select('*').eq('project_name', proj.name)
    .order('created_at', { ascending: false });
  const { data: outcomes } = await client.from('project_outcomes')
    .select('*').eq('project_name', proj.name)
    .order('sort_order');

  const activities = acts || [];
  const actNotes   = notes || [];
  const allIndUpds = indUpds || [];
  const budgetUpds = budgets || [];
  const projOutcomes = outcomes || [];

  const html = buildReportHTML(proj, activities, actNotes, allIndUpds, budgetUpds, projOutcomes, lang);
...
function buildReportHTML(proj, activities, actNotes, allIndUpds, budgetUpds, projOutcomes, lang) {
  const ID = lang === 'id';
  const t = {
    title:         ID ? 'LAPORAN PROYEK' : 'PROJECT REPORT',
    execSum:       ID ? 'RINGKASAN EKSEKUTIF' : 'EXECUTIVE SUMMARY',
    projectName:   ID ? 'Nama Proyek' : 'Project Name',
    location:      ID ? 'Lokasi' : 'Location',
    owner:         ID ? 'Pelaksana' : 'Implementing Organization',
    donor:         ID ? 'Donor / Mitra' : 'Donor / Partner',
    startDate:     ID ? 'Tanggal Mulai' : 'Start Date',
    deadline:      ID ? 'Tenggat Waktu' : 'Deadline',
    status:        ID ? 'Status' : 'Status',
    overallProgress: ID ? 'Progres Keseluruhan' : 'Overall Progress',
    budget:        ID ? 'Anggaran Disetujui' : 'Approved Budget',
    budgetActual:  ID ? 'Realisasi Anggaran' : 'Budget Realization',
    absorption:    ID ? 'Penyerapan' : 'Absorption',
    indicators:    ID ? 'CAPAIAN INDIKATOR' : 'INDICATOR ACHIEVEMENT',
    indName:       ID ? 'Indikator' : 'Indicator',
    type:          ID ? 'Jenis' : 'Type',
    target:        ID ? 'Target' : 'Target',
    actual:        ID ? 'Realisasi' : 'Actual',
    pct:           ID ? 'Capaian' : 'Achievement',
    indStatus:     ID ? 'Status' : 'Status',
    lastNote:      ID ? 'Catatan Terakhir' : 'Latest Note',
    activities:    ID ? 'DAFTAR AKTIVITAS' : 'ACTIVITY LIST',
    actName:       ID ? 'Aktivitas' : 'Activity',
    pic:           ID ? 'Penanggung Jawab' : 'Person in Charge',
    actStatus:     ID ? 'Status' : 'Status',
    actProgress:   ID ? 'Progres' : 'Progress',
    startDt:       ID ? 'Mulai' : 'Start',
    dueDt:         ID ? 'Selesai' : 'Due',
    notes:         ID ? 'Catatan' : 'Notes',
    budgetSection: ID ? 'REALISASI ANGGARAN' : 'BUDGET REALIZATION',
    budgetDate:    ID ? 'Tanggal' : 'Date',
    budgetVal:     ID ? 'Jumlah Realisasi' : 'Actual Amount',
    budgetNote:    ID ? 'Keterangan' : 'Note',
    budgetBy:      ID ? 'Diinput Oleh' : 'Recorded By',
    outcomes_:     ID ? 'OUTCOMES / HASIL' : 'OUTCOMES / RESULTS',
    narrative:     ID ? 'NARASI & PEMBELAJARAN' : 'NARRATIVE & LEARNING',
    highlights:    ID ? 'Capaian Utama Periode Ini' : 'Key Achievements This Period',
    ...
    signatures:    ID ? 'TANDA TANGAN' : 'SIGNATURES',
  ...
  const indRows = inds.map(ind => {
    ...
    const lastUpd = allIndUpds.filter(u => u.indicator_id === ind.id)[0];
    return `<tr>
      <td>${esc(ind.indicator_name)}</td>
      <td>${esc(ind.type||'Output')}</td>
      ...
      <td style="text-align:center">
        <span style="background:${color};color:#fff;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">
          ${pct>=100?(ID?'Tercapai':'Achieved'):pct>=60?(ID?'Dalam Proses':'In Progress'):(ID?'Perlu Perhatian':'Needs Attention')}
        </span>
      </td>
      <td style="font-size:11px;color:#64748b">${lastUpd && lastUpd.note ? esc(lastUpd.note) : '-'}</td>
    </tr>`;
  }).join('');

  // ---- Section: Activities ----
  const actRows = activities.map(act => {
    const actNote = actNotes.filter(n => n.activity_id === act.id).slice(0,2).map(n => `• ${esc(n.note)}`).join('<br>');
    const stColor = act.status === 'Selesai' || act.status === 'Completed' ? '#16a34a'
      : act.status === 'Berjalan' || act.status === 'In Progress' ? '#2563eb'
      : act.status === 'Terhambat' || act.status === 'Blocked' ? '#dc2626' : '#64748b';
    return `<tr>
      <td>${esc(act.title)}</td>
      <td>${esc(act.pic||'-')}</td>
      <td style="text-align:center">
        <span style="background:${stColor};color:#fff;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">${esc(act.status)}</span>
      </td>
      <td style="text-align:center">
        <div style="background:#e2e8f0;border-radius:4px;height:8px;width:80px;display:inline-block;vertical-align:middle">
          <div style="background:#2563eb;height:8px;border-radius:4px;width:${act.progress||0}%"></div>
        </div>
        <span style="font-size:11px;margin-left:4px">${act.progress||0}%</span>
      </td>
      <td style="font-size:11px">${fmtDate(act.start_date)}</td>
      <td style="font-size:11px">${fmtDate(act.due_date)}</td>
      <td style="font-size:11px;color:#475569">${actNote || '-'}</td>
    </tr>`;
  }).join('');

  // ---- Section: Budget ----
  const latestBudget = budgetUpds[0];
  const budgetRows = budgetUpds.slice(0, 10).map(b => `<tr>
    <td>${fmtDate(b.created_at)}</td>
    <td style="text-align:right;font-weight:600">${fmtMoney(b.actual_value)}</td>
    <td>${esc(b.note||'-')}</td>
    <td>${esc(b.updated_by||'-')}</td>
  </tr>`).join('');

  // ---- Section: Outcomes ----
  const outcomeRows = projOutcomes.length
    ? projOutcomes.map((o, i) => `<tr><td>${i+1}</td><td>${esc(o.outcome_text)}</td></tr>`).join('')
    : `<tr><td colspan="2" style="color:#94a3b8;text-align:center">${t.noData}</td></tr>`;

  // ---- Section: Narrative (auto-generated from notes) ----
  // Highlights: completed activities + indicators >= 100%
  ...
  const highlightItems = [
    ...completedActs.map(a => `${esc(a.title)} (${a.progress}%)`),
    ...achievedInds.map(i => `${esc(i.indicator_name)} — ${ID?'target tercapai':'target achieved'}`),
  ];

  // Challenges: blocked/stuck activities + indicators < 40%
  ...
  // Follow-up: in progress activities not yet done
  const inProgressActs = activities.filter(a => (a.status === 'Berjalan' || a.status === 'In Progress') && a.progress < 100);
  const followUpItems  = inProgressActs.map(a => `${esc(a.title)} — ${a.progress}%`);

  // Lessons: all notes from activities and indicator updates
  const allNotes = [
    ...actNotes.slice(0, 6).map(n => `[${esc(n.noted_by||'–')}] ${esc(n.note)}`),
  ...
  const mkList = arr => arr.length
    ? `<ul style="margin:0;padding-left:18px;line-height:1.9">${arr.map(i => `<li>${i}</li>`).join('')}</ul>`
    : `<p style="color:#94a3b8;font-style:italic">${t.noData}</p>`;

  // ---- Progress bar SVG ----
  const pBar = (pct, color='#2563eb') => `
    <div style="background:#e2e8f0;border-radius:6px;height:12px;width:100%;margin:4px 0">
      <div style="background:${color};height:12px;border-radius:6px;width:${Math.min(100,pct)}%;"></div>
    </div>
    <div style="font-size:11px;color:#64748b;text-align:right">${pct}%</div>`;

  return `<!DOCTYPE html>
<html lang="${lang}">
<head>
<meta charset="UTF-8">
<title>${esc(proj.name)} — ${t.title}</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .cover { text-align:center; padding: 40px 20px 30px; border-bottom: 3px solid #0f172a; margin-bottom: 24px; }
  .cover-org { font-size: 13px; font-weight: 700; color: #475569; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 8px; }
  .cover-title { font-size: 26px; font-weight: 800; color: #0f172a; margin-bottom: 4px; }
  .cover-subtitle { font-size: 14px; color: #2563eb; font-weight: 600; margin-bottom: 20px; }
  .cover-meta { display: inline-block; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 10px; padding: 16px 28px; text-align: left; }
  .cover-meta table td { padding: 4px 12px 4px 0; font-size: 11px; }
  .cover-meta table td:first-child { color: #64748b; font-weight: 600; white-space: nowrap; }
  .cover-meta table td:last-child  { font-weight: 700; color: #0f172a; }

  .section { margin-bottom: 28px; page-break-inside: avoid; }
  .section-title { font-size: 11px; font-weight: 800; letter-spacing: 1.5px; text-transform: uppercase;
    color: #fff; background: #0f172a; padding: 6px 12px; border-radius: 4px; margin-bottom: 12px; display: block; }

  .exec-grid { display: grid; grid-template-columns: repeat(3,1fr); gap: 12px; }
  .exec-card { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px 14px; }
  .exec-card .label { font-size: 10px; color: #64748b; font-weight: 600; text-transform: uppercase; margin-bottom: 4px; }
  .exec-card .value { font-size: 16px; font-weight: 800; color: #0f172a; }
  .exec-card .sub   { font-size: 10px; color: #94a3b8; margin-top: 2px; }

  table { width: 100%; border-collapse: collapse; font-size: 10.5px; }
  th { background: #f1f5f9; color: #475569; font-weight: 700; text-transform: uppercase; font-size: 10px;
       letter-spacing: .5px; padding: 7px 8px; border: 1px solid #e2e8f0; text-align: left; }
  td { padding: 7px 8px; border: 1px solid #e2e8f0; vertical-align: top; }
  tr:nth-child(even) td { background: #f8fafc; }

  .narrative-box { border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; margin-bottom: 0; }
  .narrative-row { display: grid; grid-template-columns: 1fr 1fr; }
  .narrative-cell { padding: 14px 16px; border-right: 1px solid #e2e8f0; }
  .narrative-cell:last-child { border-right: none; }
  .narrative-cell + .narrative-cell { border-top: none; }
  .narrative-cell h4 { font-size: 11px; font-weight: 700; color: #0f172a; margin-bottom: 8px;
    padding-bottom: 6px; border-bottom: 2px solid #e2e8f0; }
  .narrative-cell h4 .dot { display:inline-block;width:10px;height:10px;border-radius:50%;margin-right:6px;vertical-align:middle; }

  .sig-grid { display: grid; grid-template-columns: repeat(3,1fr); gap: 16px; margin-top: 8px; }
  .sig-box { border: 1px solid #cbd5e1; border-radius: 8px; padding: 12px 14px; text-align: center; }
  .sig-box .sig-title { font-size: 10px; font-weight: 700; color: #475569; text-transform: uppercase; margin-bottom: 4px; }
  .sig-box .sig-line  { border-top: 1px solid #cbd5e1; margin: 40px 10px 6px; }
  .sig-box .sig-label { font-size: 10px; color: #94a3b8; }
...
<div class="cover">
  ...
  <div class="cover-subtitle">${t.title}</div>
  <div class="cover-meta">
    <table>
      <tr><td>${t.projectName}</td><td>${esc(proj.name)}</td></tr>
      <tr><td>${t.location}</td><td>${esc(proj.location||'-')}</td></tr>
      <tr><td>${t.owner}</td><td>${esc(proj.owner||'-')}</td></tr>
      ${proj.donor ? `<tr><td>${t.donor}</td><td>${esc(proj.donor)}</td></tr>` : ''}
      <tr><td>${t.startDate}</td><td>${fmtDate(proj.start_date)}</td></tr>
      <tr><td>${t.deadline}</td><td>${fmtDate(proj.deadline)}</td></tr>
      <tr><td>${t.status}</td><td><strong>${esc(proj.status)}</strong></td></tr>
      <tr><td>${t.printedOn}</td><td>${today}</td></tr>
    </table>
  </div>
</div>
...
<!-- ===== 1. EXECUTIVE SUMMARY ===== -->
<div class="section">
  <span class="section-title">1. ${t.execSum}</span>
  <div class="exec-grid">
    <div class="exec-card">
      <div class="label">${t.overallProgress}</div>
      <div class="value">${overall}%</div>
      ${pBar(overall, overall>=70?'#16a34a':overall>=40?'#d97706':'#dc2626')}
    </div>
    <div class="exec-card">
      <div class="label">${t.budget}</div>
      <div class="value" style="font-size:13px">${fmtMoney(proj.budget_approved)}</div>
      <div class="sub">${t.budgetActual}: ${fmtMoney(proj.budget_actual)}</div>
      ${pBar(budgetPct, '#7c3aed')}
    </div>
    <div class="exec-card">
      <div class="label">${t.indicators}</div>
      <div class="value">${inds.length} ${ID?'Indikator':'Indicators'}</div>
      <div class="sub">${achievedInds.length} ${ID?'tercapai':'achieved'} · ${activities.length} ${ID?'aktivitas':'activities'}</div>
      ${pBar(inds.length ? Math.round(achievedInds.length/inds.length*100) : 0, '#2563eb')}
    </div>
  </div>
  ${proj.description ? `<div style="margin-top:12px;padding:10px 14px;background:#f8fafc;border-left:3px solid #2563eb;border-radius:0 6px 6px 0;font-size:11px;color:#475569"><strong>${t.description}:</strong> ${esc(proj.description)}</div>` : ''}
</div>

<!-- ===== 2. INDICATORS ===== -->
<div class="section">
  <span class="section-title">2. ${t.indicators}</span>
  ${inds.length ? `<table>
    <thead><tr>
      <th style="width:26%">${t.indName}</th><th>${t.type}</th>
      <th>${t.target}</th><th>${t.actual}</th>
      <th>${t.pct}</th><th>${t.indStatus}</th><th style="width:22%">${t.lastNote}</th>
    </tr></thead>
    <tbody>${indRows}</tbody>
  </table>` : `<p style="color:#94a3b8;font-style:italic;font-size:11px">${t.noData}</p>`}
</div>

<!-- ===== 3. ACTIVITIES ===== -->
<div class="section">
  <span class="section-title">3. ${t.activities}</span>
  ${activities.length ? `<table>
    <thead><tr>
      <th style="width:24%">${t.actName}</th><th>${t.pic}</th>
      <th>${t.actStatus}</th><th>${t.actProgress}</th>
      <th>${t.startDt}</th><th>${t.dueDt}</th>
...
<!-- ===== 4. BUDGET ===== -->
<div class="section">
  <span class="section-title">4. ${t.budgetSection}</span>
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:12px">
    <div class="exec-card"><div class="label">${t.budget}</div><div class="value" style="font-size:13px">${fmtMoney(proj.budget_approved)}</div></div>
    <div class="exec-card"><div class="label">${t.budgetActual}</div><div class="value" style="font-size:13px">${fmtMoney(proj.budget_actual)}</div></div>
    <div class="exec-card"><div class="label">${t.absorption}</div><div class="value">${budgetPct}%</div>${pBar(budgetPct,'#7c3aed')}</div>
  </div>
  ${budgetRows ? `<table>
    <thead><tr><th>${t.budgetDate}</th><th>${t.budgetVal}</th><th>${t.budgetNote}</th><th>${t.budgetBy}</th></tr></thead>
...
<!-- ===== 5. OUTCOMES ===== -->
<div class="section">
  <span class="section-title">5. ${t.outcomes_}</span>
  <table>
    <thead><tr><th style="width:5%">#</th><th>${t.outcomes_}</th></tr></thead>
...
<div class="section">
  <span class="section-title">6. ${t.narrative}</span>
  <div class="narrative-box">
    <div class="narrative-row">
      <div class="narrative-cell">
        <h4><span class="dot" style="background:#16a34a"></span>${t.highlights}</h4>
        ${mkList(highlightItems)}
      </div>
      <div class="narrative-cell">
        <h4><span class="dot" style="background:#dc2626"></span>${t.challenges}</h4>
        ${mkList(challengeItems)}
      </div>
    </div>
    <div class="narrative-row" style="border-top:1px solid #e2e8f0">
      <div class="narrative-cell">
        <h4><span class="dot" style="background:#2563eb"></span>${t.followUp}</h4>
        ${mkList(followUpItems)}
      </div>
      <div class="narrative-cell">
        <h4><span class="dot" style="background:#d97706"></span>${t.lessons}</h4>
        ${mkList(allNotes)}
      </div>
    </div>
  </div>
...
<div class="section no-break">
  <span class="section-title">7. ${t.signatures}</span>
  <div class="sig-grid">
    <div class="sig-box">
      <div class="sig-title">${t.sigExec}</div>
      <div class="sig-line"></div>
      <div class="sig-label">${t.sigName}</div>
      <div style="margin-top:6px;border-top:1px dashed #e2e8f0;padding-top:6px;font-size:10px;color:#94a3b8">${t.sigDate}: ______________________</div>
    </div>
    <div class="sig-box">
      <div class="sig-title">${t.sigSup}</div>
      <div class="sig-line"></div>
      <div class="sig-label">${t.sigName}</div>
      <div style="margin-top:6px;border-top:1px dashed #e2e8f0;padding-top:6px;font-size:10px;color:#94a3b8">${t.sigDate}: ______________________</div>
    </div>
    <div class="sig-box">
      <div class="sig-title">${t.sigMgr}</div>
      <div class="sig-line"></div>
      <div class="sig-label">${t.sigName}</div>
      <div style="margin-top:6px;border-top:1px dashed #e2e8f0;padding-top:6px;font-size:10px;color:#94a3b8">${t.sigDate}: ______________________</div>
    </div>
...
<!-- ===== FOOTER ===== -->
<div class="footer">
  <span>${esc(proj.name)} — ${t.title}</span>
  <span>${t.printedOn}: ${today}</span>
</div>
